...
| Behavioral RLS + financial RPCs | Real queries as two tenants: cross-company SELECT/INSERT/UPDATE/DELETE are blocked; role gates work; `record_invoice_payment_atomic` rejects foreign invoices, zero amounts and anonymous callers; browser cannot insert into `journal_lines`; idempotency table has its unique key; SECURITY DEFINER functions are not `anon`-executable; append-only DELETE triggers fire. |
