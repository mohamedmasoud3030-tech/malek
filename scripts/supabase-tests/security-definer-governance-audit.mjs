...
async function main() {
  ...
  // These functions have verified ADMIN/MANAGER boundaries in the canonical
  // baseline. Stabilization changes the authority source to company_members.role
  // without redesigning which roles may execute their sensitive business action.
  ...
  const managerGateTargets = [
    ...
    ['SD-04', 'public.record_invoice_payment_atomic(jsonb)'],
    ['SD-05', 'public.record_invoice_payment_atomic_engine(jsonb)'],
  ...
  const grantCases = [
    ['SD-17', 'authenticated', 'public.post_receipt_atomic(jsonb)', false],
    ['SD-18', 'authenticated', 'public.record_invoice_payment_atomic_engine(jsonb)', false],
    ['SD-19', 'authenticated', 'public.execute_receipt_void_internal(jsonb)', false],
    ['SD-20', 'authenticated', 'public.preview_bank_statement_batch_atomic(jsonb)', true],
    ['SD-21', 'authenticated', 'public.import_bank_statement_batch_atomic(jsonb)', true],
    ['SD-22', 'authenticated', 'public.record_invoice_payment_atomic(jsonb)', true],
