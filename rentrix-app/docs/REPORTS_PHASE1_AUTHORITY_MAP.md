...
Phase 2+ must extend this stack rather than introduce a second renderer/export engine.

## Workspace boundary

`report-workspaces.ts` owns report IA configuration. `workspace/adapters/report-adapter-contract.ts` is the rendering extension seam. Presentation refactors should preserve the router/workspace contract and move shared visual behavior into report-specific presentation primitives rather than modifying unrelated app-wide primitives.
...
The following labels are not sufficient as canonical concepts and must be disambiguated in Phase 2 view models/UI copy:
