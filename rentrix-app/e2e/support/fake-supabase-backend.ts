...
export function buildAcceptanceSeed(mode: CompanySettingsMode): AcceptanceSeed {
  ...
  const rpcs: Record<string, (args: Record<string, unknown>) => unknown> = {
    ...
    /**
     * R1 authoritative dashboard read model. Seeded explicitly with zeroed
     * figures so the shell renders without inventing financial data that any
     * document could pick up.
     */
    rpt_dashboard_snapshot: () => ({
      meta: { source: 'rpt_dashboard_snapshot' },
      portfolio: { properties: 1, units: 1 },
      occupancy: { occupied_units: 1, vacant_units: 0, occupancy_rate: 100 },
