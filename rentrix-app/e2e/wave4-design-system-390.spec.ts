...
const surfaces = [
  ...
  {
    name: 'reports',
    url: '/login?e2e-reports-products=1',
    ready: 'main[data-e2e-reports-products]',
  },
  {
    name: 'owners',
...
async function assertResponsiveCardGrids(page: Page, label: string) {
  ...
  for (let index = 0; index < count; index += 1) {
    ...
    expect(
      columns,
      `${label}: responsive card grid ${index} must use two mobile columns`,
    ).toBe(2);
  }
}
...
test.beforeEach(async ({}, testInfo) => {
  test.skip(
    ...
    'Wave 4 390px acceptance runs once in chromium-desktop shard',
  );
});

for (const surface of surfaces) {
  test(`Wave 4 ${surface.name} — 390px responsive contract`, async ({
    page,
  }) => {
