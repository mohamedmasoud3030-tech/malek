...
async function preparedContext(browser: import('@playwright/test').Browser, options: { width: number; height: number; theme?: 'light' | 'dark' }): Promise<BrowserContext> {
  ...
  await context.route('https://fonts.googleapis.com/**', (route) =>
    route.fulfill({ status: 200, contentType: 'text/css; charset=utf-8', body: fontCss ?? '' }));
  await context.route('https://fonts.gstatic.com/**', (route) => route.fulfill({ status: 204, body: '' }));
  const theme = options.theme ?? 'light';
  await context.addInitScript(({ session, selectedTheme }) => {
    try {
    ...
    } catch { /* storage unavailable — the guards below fail visibly */ }
    ...
    Object.defineProperty(window, 'open', {
      ...
      value: (...openArgs: Parameters<typeof originalOpen>) => {
        ...
        if (opened) {
          ...
          try {
            Object.defineProperty(opened, 'print', { configurable: true, value: () => undefined });
          } catch { /* headless without the stub — the popup may close early and the assertions fail visibly */ }
          try {
            Object.defineProperty(opened, 'close', { configurable: true, value: () => undefined });
          } catch { /* same — assertions stay honest */ }
        }
        return opened;
      },
    });
...
async function capture(page: Page, name: string, options: { viewportOnly?: boolean } = {}): Promise<void> {
  mkdirSync(EVIDENCE_DIR, { recursive: true });
  await page.waitForTimeout(450);
  await page.screenshot({ path: `${EVIDENCE_DIR}/${name}.png`, fullPage: !options.viewportOnly });
}

const summary: Record<string, unknown> = {};

test('premium reports catalog — full product journey with print/PDF/share evidence', async ({ browser }) => {
  ...
  await expect(page.locator('[data-report-product="owner-comprehensive-statement"]')).toContainText('كشف المالك الشامل');
  summary.landing = {
    ...
    money: await page.evaluate(() => /\d{3,}(?:[.,]\d+)?\s*(OMR|ر\.ع)/i.test(document.body.innerText)),
  };
  ...
  await page.locator('[data-report-product="owner-comprehensive-statement"] button').first().focus();
  await page.keyboard.press('Enter');
  await page.waitForURL('**/reports/owner-comprehensive-statement');
  facts.push({ id: 'catalog-opens-real-route', url: new URL(page.url()).pathname });
  await context.close();

  /* 2 — mobile catalog: two columns at 390 and 360, no horizontal overflow. */
  for (const [width, theme, name] of [[390, 'light', '03-catalog-mobile-390-light'], [360, 'dark', '04-catalog-mobile-360-dark']] as const) {
  ...
  page = await context.newPage();
  await seed(page);
  await open(page, `/reports/owner-comprehensive-statement?ownerId=${IDS.owner}&from=2026-01-01&to=2026-12-31`);
  await expect(page.locator('[data-report-product-page="owner-comprehensive-statement"]')).toBeVisible();
  const headerText = (await page.locator('[data-report-product-header]').innerText()).replace(/\s+/g, ' ');
  ...
  const printDoc = await popup.evaluate(() => ({
    ...
    companyOnDocument: document.body.innerText.includes('شركة الأفق لإدارة الأملاك'),
    omrThreeDecimals: (document.body.innerText.match(/\d+\.\d{3}/g) ?? []).length,
    avoidBreakRules: [...document.querySelectorAll('[style]')].some((el) => (el.getAttribute('style') ?? '').includes('break-inside')),
  }));
  ...
  await popup.screenshot({ path: `${EVIDENCE_DIR}/06-owner-print-popup-top.png` });
  await popup.evaluate(() => window.scrollTo(0, Math.max(0, document.body.scrollHeight * 0.5)));
  await popup.waitForTimeout(400);
  await popup.screenshot({ path: `${EVIDENCE_DIR}/07-owner-print-popup-tables.png` });
  ...
  const fallback = await page.evaluate(() => {
    ...
    return clipPromise.then((clip) => ({
      mentionsSecureLink: toastText.includes('الرابط الآمن') || toastText.includes('تم نسخ رابط التقرير'),
  ...
  context = await preparedContext(browser, { width: 1440, height: 1000, theme: 'dark' });
  page = await context.newPage();
  await seed(page);
  await open(page, `/reports/tenant-statement?contractId=${IDS.contract}&from=2026-01-01&to=2026-12-31`);
  ...
  page = await context.newPage();
  await seed(page);
  await open(page, '/reports/collections-arrears-cheques?view=period&from=2026-01-01&to=2026-12-31');
  facts.push({ id: 'collections-tabs', tabs: await page.locator('[data-report-product-tabs] [role="tab"]').count() });
  await capture(page, '11-collections-period');
  await open(page, '/reports/collections-arrears-cheques?view=period&from=2020-01-01&to=2020-01-31');
  ...
  page = await context.newPage();
  await seed(page);
  await open(page, '/reports/portfolio-property-performance?view=office&from=2026-01-01&to=2026-12-31');
  facts.push({ id: 'portfolio-visuals', visualsInsideDetail: await page.locator('[data-report-product-workspace] svg, [data-report-product-workspace] canvas').count() });
  ...
  page = await context.newPage();
  await seed(page);
  await open(page, '/reports/financial-settlement-pack?view=statements&asOf=2026-12-31');
  facts.push({ id: 'financial-pack', trialBalanceVisible: await page.getByText('ميزان المراجعة').first().isVisible().catch(() => false) });
  await page.getByText('ميزان المراجعة').first().scrollIntoViewIfNeeded().catch(() => undefined);
  await capture(page, '14-financial-pack', { viewportOnly: true });
  await context.close();

  /* 8 — legacy deep links keep the consolidated workspace alive. */
  context = await preparedContext(browser, { width: 1440, height: 1000 });
  page = await context.newPage();
  await seed(page);
  await open(page, '/reports?section=accounting&view=general_ledger&from=2026-01-01&to=2026-12-31');
  facts.push({
    id: 'legacy-compat',
    primaryNav: await page.locator('[data-reports-primary-navigation]').count(),
    catalogOnLegacy: await page.locator('[data-reports-catalog-landing]').count(),
  });
  await capture(page, '15-legacy-compat-workspace');

  await open(page, '/reports/ghost-report-404');
  facts.push({ id: 'unknown-product', honestState: await page.locator('[data-report-product-not-found]').count() });
  ...
  await seed(page);
  await open(page, `/reports/owner-comprehensive-statement?ownerId=${IDS.owner}&from=2026-01-01&to=2026-12-31`);
  facts.push({ id: 'owner-mobile', overflow: await page.evaluate(() => document.documentElement.scrollWidth - document.documentElement.clientWidth) });
  await capture(page, '17-owner-report-mobile-390');
  await context.close();

  mkdirSync(EVIDENCE_DIR, { recursive: true });
  writeFileSync(`${EVIDENCE_DIR}/evidence.json`, JSON.stringify({ summary, facts }, null, 2));
  // The catalog contract itself is asserted hard — if it regressed to a
  // dashboard, this evidence run must fail.
  expect(summary.landing).toEqual(expect.objectContaining({ cards: 5, kpiLayer: 0, charts: 0, filterSurface: 0, money: false }));
});
