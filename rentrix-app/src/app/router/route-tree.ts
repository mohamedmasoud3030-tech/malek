...
const accountingRoute = createRoute({ getParentRoute: () => protectedRoute, path: '/accounting', beforeLoad: () => { throw redirect({ to: '/reports', search: (previous: Record<string, unknown>) => ({ ...previous, section: 'general_ledger' }) }); }, staticData: { title: 'دفتر الأستاذ والشجرة' } });
