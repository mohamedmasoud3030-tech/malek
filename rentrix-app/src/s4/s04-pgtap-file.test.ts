/**
 * Runs supabase/tests/property_management_gl_lifecycle.sql against a full PGlite
 * replay using the faithful pgTAP shim.
 */
...
describe('Stage S04 pgTAP gate file (faithful shim)', () => {
  it('runs supabase/tests/property_management_gl_lifecycle.sql with zero failures', async () => {
    ...
    const stmts = splitStatements(transformSuite(readFileSync(join(repoRoot, 'supabase', 'tests', 'property_management_gl_lifecycle.sql'), 'utf8')));
    ...
    if (topErrors.length > 0) {
      console.error('topErrors:', topErrors);
    }

    const { rows } = await db.query<{ line: string }>('select line from _s04_test_results order by seq');
    const failed = rows.filter((r) => r.line.startsWith('not ok')).map((r) => r.line);
