 ...
 *
 * Exercises the real S06 RPCs and canonical S03 posting engine rather than
 * inspecting function source only.
 */
...
beforeAll(async () => {
  ...
  await db.exec(`
    ...
    insert into public.contracts(
      ...
      rent_amount,status,company_id
    ) values(
      '${CONTRACT_A}','${PROPERTY_A}','${UNIT_A}','${TENANT_A}','${AGREEMENT_A}',
      date '2026-08-01',date '2027-07-31',80,'active','${COMPANY_A}'
    ) on conflict(id) do nothing;
...
describe('S06 master lease production lifecycle', () => {
  ...
  it('rejects cross-company use of another company master-lease agreement', async () => {
    await expect(
      rpc('gl_ml_create_initial_measurement', {
  ...
  it('creates one idempotent server-derived initial measurement', async () => {
    ...
    const { rows } = await db.query<{ count: string }>(
      `select count(*)::text as count
        ...
        where company_id=$1::uuid and owner_agreement_id=$2::uuid`,
      [COMPANY_A, AGREEMENT_A],
    );
    expect(Number(rows[0]?.count)).toBe(1);
  });

  it('posts initial recognition and first period to the canonical GL', async () => {
    const initial = await rpc('gl_ml_post_initial_recognition', {
      company_id: COMPANY_A,
    ...
    expect(await accountNetDebit(COMPANY_A, '6300')).toBe(0);
    expect(await accountNetDebit(COMPANY_A, '1120')).toBe(-100);
  });

  it('freezes a remeasurement at the posted boundary and blocks old future posting', async () => {
    ...
    expect(draft.liability_delta).toBe(20);
    expect(draft.rou_adjustment).toBe(20);

    await expect(
      rpc('gl_ml_post_period', {
        company_id: COMPANY_A,
        measurement_id: initialMeasurementId,
        period_no: 2,
        cash_account_no: '1120',
  ...
  it('posts the remeasurement and rebases future depreciation to new carrying ROU', async () => {
    const posted = await rpc('gl_ml_post_remeasurement', {
      company_id: COMPANY_A,
  ...
  it('posts gross sublease revenue without touching Owner Funds Payable', async () => {
  ...
  it('fully terminates at the next posted boundary and clears net ROU/liability', async () => {
    ...
    expect(termination.liability_delta).toBe(-60);
    expect(termination.rou_adjustment).toBe(-60);
    expect(termination.termination_gain_loss).toBe(0);

    const posted = await rpc('gl_ml_post_remeasurement', {
      company_id: COMPANY_A,
      measurement_id: String(termination.measurement_id),
    });
    expect(posted.status).toBe('TERMINATED');
