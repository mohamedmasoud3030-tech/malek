...
INSERT INTO public.companies (id, name, slug) VALUES
  ('${COMPANY_A}', 'شركة ألف', 'alpha'), ('${COMPANY_B}', 'شركة باء', 'beta');
INSERT INTO auth.users (id, email) VALUES
  ('${ADMIN_A}', 'admin.a@p0.test'), ('${MANAGER_B}', 'manager.b@p0.test'), ('${USER_B}', 'user.b@p0.test');
...
INSERT INTO public.company_members (company_id, user_id, role) VALUES
  ('${COMPANY_A}', '${ADMIN_A}', 'ADMIN'), ('${COMPANY_B}', '${MANAGER_B}', 'ADMIN'), ('${COMPANY_B}', '${USER_B}', 'MEMBER');
...
INSERT INTO public.receipts (id, amount, status, company_id) VALUES
  ('${RECEIPT_A}', ${A_PAY}, 'POSTED', '${COMPANY_A}'), ('${RECEIPT_B}', ${B_PAY}, 'POSTED', '${COMPANY_B}');
...
type Probe = { fn: string; kind: 'report-leak' | 'exploit' | 'guard'; actor: string; asCompany: string | null; before: unknown; after?: unknown };
const probes: Probe[] = [];
const errors: string[] = [];
let db: PGlite;
...
async function probeReport(db: PGlite, when: 'before' | 'after') {
  ...
  const runAs = async (fn: string, kind: Probe['kind'], sql: string, params: unknown[] = []) => {
    ...
    let data: unknown;
    try { data = await call(db, sql, params); } catch (e) { data = { error: String(e).slice(0, 200) }; }
...
async function probeExploitAndGuards(db: PGlite, when: 'before' | 'after') {
  await assume(db, ADMIN_A, COMPANY_A);
  const fabricated = { request_id: '9a000000-0000-4000-8000-0000000000e1', owner_id: OWNER_A, property_id: PROPERTY_A, period_start: '2026-07-01', period_end: '2026-07-31', gross_collected: 999999, office_fee: 0, owner_expenses: 0, tax_amount: 0, notes: 'p0-exploit-proof' };
  let exploit: unknown;
  await db.exec('BEGIN;');
  try { exploit = await call(db, `SELECT public.create_owner_settlement_draft_atomic($1::jsonb) AS out`, [JSON.stringify(fabricated)]); } catch (e) { exploit = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  ...
  await db.exec('BEGIN;');
  try { crossOwner = await call(db, `SELECT public.create_owner_settlement_draft_atomic($1::jsonb) AS out`, [JSON.stringify({ ...fabricated, request_id: '9a000000-0000-4000-8000-0000000000e2', owner_id: OWNER_B, property_id: PROPERTY_B })]); } catch (e) { crossOwner = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  ...
  let lowRole: unknown;
  await db.exec('BEGIN;');
  try { lowRole = await call(db, `SELECT public.create_owner_settlement_draft_atomic($1::jsonb) AS out`, [JSON.stringify({ ...fabricated, request_id: '9a000000-0000-4000-8000-0000000000e3', owner_id: OWNER_B, property_id: PROPERTY_B })]); } catch (e) { lowRole = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  recordProbe('create_owner_settlement_draft_atomic[low-role-user]', 'guard', 'userB@B', 'B', when, lowRole);

  await assume(db, ADMIN_A, COMPANY_A);
  let crossWrite: unknown;
  await db.exec('BEGIN;');
  try { crossWrite = await call(db, `SELECT public.record_invoice_payment_atomic($1::jsonb) AS out`, [JSON.stringify({ request_id: '9c000000-0000-4000-8000-0000000000e4', invoice_id: INVOICE_B, amount: 5, method: 'cash', date: '2026-07-21', reference: 'p0-cross-tenant-write' })]); } catch (e) { crossWrite = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  recordProbe('record_invoice_payment_atomic[cross-company-invoice]', 'guard', 'adminA@A', 'A', when, crossWrite);

  await assume(db, ADMIN_A, COMPANY_A);
  let directPost: unknown;
  await db.exec('BEGIN;');
  try { directPost = await call(db, `SELECT public.post_receipt_atomic($1::jsonb) AS out`, [JSON.stringify({ request_id: '9c000000-0000-4000-8000-0000000000e5', receipt: { id: '9b000000-0000-4000-8000-0000000000e5', contract_id: CONTRACT_B, date_time: '2026-07-21', channel: 'cash', amount: 5, ref: 'p0-cross-post', status: 'POSTED' }, allocations: [{ id: '9e000000-0000-4000-8000-0000000000e5', invoice_id: INVOICE_B, amount: 5 }], journal_entries: [] })]); } catch (e) { directPost = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  recordProbe('post_receipt_atomic[cross-company-contract]', 'guard', 'adminA@A', 'A', when, directPost);

  await assume(db, ADMIN_A, COMPANY_A);
  let agrOwn: unknown;
  await db.exec('BEGIN;');
  try { agrOwn = await call(db, `SELECT public.create_owner_agreement_atomic($1::jsonb) AS out`, [JSON.stringify({ owner_id: OWNER_A, property_id: PROPERTY_A2, agreement_type: 'property_management', commission_type: 'RATE', commission_value: 12, starts_on: '2026-08-01' })]); } catch (e) { agrOwn = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  recordProbe('create_owner_agreement_atomic[own-refs]', 'guard', 'adminA@A', 'A', when, agrOwn);

  await assume(db, ADMIN_A, COMPANY_A);
  let agrCross: unknown;
  await db.exec('BEGIN;');
  try { agrCross = await call(db, `SELECT public.create_owner_agreement_atomic($1::jsonb) AS out`, [JSON.stringify({ owner_id: OWNER_B, property_id: PROPERTY_B, agreement_type: 'property_management', commission_type: 'RATE', commission_value: 12, starts_on: '2026-08-01' })]); } catch (e) { agrCross = { error: String(e).slice(0, 250) }; } finally { await db.exec('ROLLBACK;'); }
  recordProbe('create_owner_agreement_atomic[cross-refs]', 'guard', 'adminA@A', 'A', when, agrCross);

  await assume(db, ADMIN_A, COMPANY_A);
  await db.exec('SET ROLE authenticated;');
  let osCross: unknown;
  try { osCross = await call(db, `SELECT public.rpt_owner_statement('${OWNER_B}'::uuid, '2026-07-01'::date, '2026-07-31'::date) AS out`); } catch (e) { osCross = { error: String(e).slice(0, 200) }; }
  await db.exec('RESET ROLE;');
  ...
  const inTx = async (fn: () => Promise<unknown>) => { await db.exec('BEGIN;'); try { return await fn(); } finally { await db.exec('ROLLBACK;'); } };
  try {
    rest.selectPayments = await inTx(async () => (await call(db, `SELECT count(*)::int AS n FROM public.payments`))[0]?.n);
    const targets = await inTx(async () => ({ payment: (await call(db, `SELECT id::text AS id FROM public.payments WHERE company_id = '${COMPANY_B}' ORDER BY 1 LIMIT 1`))[0]?.id ?? null, expense: (await call(db, `SELECT id::text AS id FROM public.expenses WHERE company_id = '${COMPANY_B}' ORDER BY 1 LIMIT 1`))[0]?.id ?? null })) as { payment: string | null; expense: string | null };
    rest.foreignTargets = targets;
    if (targets.expense) {
      rest.updateForeign = await inTx(async () => { try { return (await call(db, `WITH u AS (UPDATE public.expenses SET amount = amount + 1 WHERE id = '${targets.expense}' RETURNING id) SELECT count(*)::int AS n FROM u`))[0]?.n; } catch (e) { return { error: String(e).slice(0, 150) }; } });
      rest.deleteForeign = await inTx(async () => { try { return (await call(db, `WITH d AS (DELETE FROM public.expenses WHERE id = '${targets.expense}' RETURNING id) SELECT count(*)::int AS n FROM d`))[0]?.n; } catch (e) { return { error: String(e).slice(0, 150) }; } });
    } else { rest.updateForeign = 'no-foreign-row-visible'; rest.deleteForeign = 'no-foreign-row-visible'; }
    rest.insertSpoofExpense = await inTx(async () => { try { await call(db, `INSERT INTO public.expenses (id, property_id, category, amount, expense_date, status, charged_to, company_id) VALUES (gen_random_uuid(), '${PROPERTY_B}', 'p0-spoof', 7, '2026-07-22', 'POSTED', 'office', '${COMPANY_B}')`); return 'inserted'; } catch (e) { return String(e).slice(0, 180); } });
    rest.insertOwnExpense = await inTx(async () => { try { await call(db, `INSERT INTO public.expenses (id, property_id, category, amount, expense_date, status, charged_to, company_id) VALUES (gen_random_uuid(), '${PROPERTY_A}', 'p0-own', 7, '2026-07-22', 'POSTED', 'office', '${COMPANY_A}')`); return 'inserted'; } catch (e) { return String(e).slice(0, 180); } });
  ...
  try { anonCall = await call(db, `SELECT public.rpt_cash_flow('2026-07-01'::date, '2026-07-31'::date) AS out`); } catch (e) { anonCall = { error: String(e).slice(0, 200) }; }
  await db.exec('RESET ROLE;');
  ...
  for (const t of ['payments', 'expenses', 'owner_settlements', 'invoices', 'contracts']) {
    try { const r = await call(db, `SELECT count(*)::int AS n FROM public.${t}`); rlsCounts[t] = r[0]?.n; } catch (e) { rlsCounts[t] = { error: String(e).slice(0, 150) }; }
  }
  await db.exec('RESET ROLE;');
...
async function grantCatalog(db: PGlite) {
  return call(db, `SELECT r.routine_name AS name, r.security_type AS security, coalesce(p.proacl::text, '') AS acl FROM information_schema.routines r JOIN pg_proc p ON p.proname = r.routine_name JOIN pg_namespace n ON n.oid = p.pronamespace AND n.nspname = 'public' WHERE r.specific_schema = 'public' AND (r.routine_name LIKE 'rpt\\_%' ESCAPE '\\' OR r.routine_name LIKE '%\\_atomic' ESCAPE '\\') ORDER BY 1`);
}
...
function fnProbe(fn: string) { const p = probes.find((x) => x.fn === fn); expect(p, `probe recorded for ${fn}`).toBeTruthy(); return p as Probe; }
function outOf(p: Probe, when: 'before' | 'after'): any { const rows: any = p[when]; expect(rows, `${p.fn} ${when} rows`).toBeTruthy(); expect(rows?.[0]?.out?.error, `${p.fn} ${when} must not error: ${JSON.stringify(rows?.[0]?.out?.error ?? '')}`).toBeUndefined(); return rows[0].out; }
...
beforeAll(async () => {
  ...
  for (const file of files) {
    ...
    if (file === '20260713000005_fix_void_receipt_anon_grant.sql') sql = sql.replace(/RAISE EXCEPTION 'Post-flight check/gi, "RAISE WARNING 'Post-flight check");
    if (FIX_FILE && file === FIX_FILE) {
      sql = sql.replace(/alter table public\."journal_entries" enable row level security;[\s\S]*?drop policy if exists p0_tenant_isolation on public\."journal_entries";[\s\S]*?create policy p0_tenant_isolation on public\."journal_entries" as restrictive[\s\S]*?using \(company_id = public\.current_company_id\(\)\)[\s\S]*?with check \(company_id = public\.current_company_id\(\)\);/, () => `do $$ begin if to_regclass('public.journal_entries') is not null and (select relkind from pg_class where oid = 'public.journal_entries'::regclass) = 'r' then execute 'alter table public."journal_entries" enable row level security'; execute 'drop policy if exists p0_tenant_isolation on public."journal_entries"'; execute 'create policy p0_tenant_isolation on public."journal_entries" as restrictive using (company_id = public.current_company_id()) with check (company_id = public.current_company_id())'; end if; end $$;`);
      continue;
    }
    try { await db.exec(sql); } catch (e) { errors.push(`${file}: ${String(e).slice(0, 200)}`); await db.exec('ROLLBACK;').catch(() => undefined); }
  }
...
afterAll(() => {
  if (process.env.WRITE_EVIDENCE === 'true') {
    ...
    writeFileSync(join(evidenceDir, 'behavioral-isolation.json'), JSON.stringify({ generatedAt: new Date().toISOString(), fixFile: FIX_FILE, replayErrors: errors, fixture: { A_PAY, B_PAY, A_EXP, B_EXP, B_JE, B_RENT }, probes }, null, 2));
  }
});
...
describe('P0 — post-fix verification (only when the P0 fix migration exists)', () => {
  it.skipIf(!FIX_FILE)('applies the P0 fix migration cleanly on top of replayed main', async () => {
    ...
    try {
      ...
      fixSql = fixSql.replace(/alter table public\."journal_entries" enable row level security;[\s\S]*?drop policy if exists p0_tenant_isolation on public\."journal_entries";[\s\S]*?create policy p0_tenant_isolation on public\."journal_entries" as restrictive[\s\S]*?using \(company_id = public\.current_company_id\(\)\)[\s\S]*?with check \(company_id = public\.current_company_id\(\)\);/, () => `do $$ begin if to_regclass('public.journal_entries') is not null and (select relkind from pg_class where oid = 'public.journal_entries'::regclass) = 'r' then execute 'alter table public."journal_entries" enable row level security'; execute 'drop policy if exists p0_tenant_isolation on public."journal_entries"'; execute 'create policy p0_tenant_isolation on public."journal_entries" as restrictive using (company_id = public.current_company_id()) with check (company_id = public.current_company_id())'; end if; end $$;`);
  ...
  it.skipIf(!FIX_FILE)('closes every report leak with numeric parity vs ground truth (company A only)', async () => {
    ...
    expect(num(daily?.total)).toBe(A_PAY); expect(daily?.rows).toHaveLength(1); expect(num(daily?.rows?.[0]?.total)).toBe(A_PAY); expect(num(daily?.rows?.[0]?.count)).toBe(1);
    const dash = outOf(fnProbe('rpt_dashboard_overview'), 'after');
    expect(num(dash?.financial?.total_collected)).toBe(A_PAY); expect(num(dash?.financial?.total_expenses)).toBe(A_EXP); expect(num(dash?.operational?.properties)).toBe(2); expect(num(dash?.operational?.units)).toBe(1); expect(num(dash?.operational?.activeContracts)).toBe(1); expect(num(dash?.operational?.overdueInvoices)).toBe(0);
    const fsRows: any = fnProbe('rpt_financial_summary').after; const fs = fsRows?.[0]; expect(fs).toBeTruthy(); expect(num(fs?.collected)).toBe(A_PAY); expect(num(fs?.expenses)).toBe(A_EXP); expect(num(fs?.active_contracts)).toBe(1); expect(num(fs?.total_units)).toBe(1);
    const inc = outOf(fnProbe('rpt_income_statement'), 'after'); expect(num(inc?.total_revenue)).toBe(A_PAY); expect(num(inc?.total_expenses)).toBe(A_EXP); expect(num(inc?.net_income)).toBe(A_PAY - A_EXP);
    const vat = outOf(fnProbe('rpt_vat_return'), 'after'); expect(num(vat?.total_sales_amount)).toBe(A_PAY); expect(num(vat?.total_tax_amount)).toBe(0); expect(num(vat?.invoice_count)).toBe(1);
    const BROKEN_CALLS: Record<string, { sql: string; params?: unknown[] }> = {
      rpt_trial_balance: { sql: `SELECT public.rpt_trial_balance('2026-07-31'::date)` }, rpt_balance_sheet: { sql: `SELECT public.rpt_balance_sheet('2026-07-31'::date)` }, rpt_aged_receivables: { sql: `SELECT public.rpt_aged_receivables('2026-07-31'::date)` }, rpt_overdue_invoices: { sql: `SELECT public.rpt_overdue_invoices('2026-07-31'::date)` }, rpt_rent_roll: { sql: `SELECT public.rpt_rent_roll('2026-07-31'::date)` }, rpt_tenant_statement: { sql: `SELECT public.rpt_tenant_statement($1::uuid)`, params: [CONTRACT_A] },
    };
    const preExisting: Record<string, unknown> = {};
    for (const [fn, spec] of Object.entries(BROKEN_CALLS)) {
      const pr = fnProbe(fn); const beforeErr = (pr.before as any)?.error ?? JSON.stringify(pr.before); const afterErr = (pr.after as any)?.error ?? JSON.stringify(pr.after); let superuserErr: string;
      try { const r = await call(db, spec.sql, spec.params ?? []); superuserErr = `UNEXPECTED SUCCESS: ${JSON.stringify(r).slice(0, 160)}`; } catch (e) { superuserErr = String(e).slice(0, 200); }
      preExisting[fn] = { error_authenticated_before: beforeErr, error_authenticated_after: afterErr, error_superuser: superuserErr, securityImpact: 'none — call fails closed (error), no company data returned in any context' };
      expect(afterErr).toBe(beforeErr); expect(String(beforeErr)).not.toMatch(/^\[\{"out"/); expect(superuserErr).not.toMatch(/^UNEXPECTED/); expect(superuserErr.replace(/^error: /, '')).toContain(String(beforeErr).replace(/^error: /, '').slice(0, 40));
    }
    mkdirSync(evidenceDir, { recursive: true }); writeFileSync(join(evidenceDir, 'pre-existing-defects.json'), JSON.stringify({ generatedAt: new Date().toISOString(), functions: preExisting }, null, 2));
    const os = outOf(fnProbe('rpt_owner_statement'), 'after'); const osBefore = outOf(fnProbe('rpt_owner_statement'), 'before'); expect(num(os?.total_gross)).toBe(num(osBefore?.total_gross)); expect(num(os?.total_deductions)).toBe(num(osBefore?.total_deductions)); expect(num(os?.total_net)).toBe(num(osBefore?.total_net)); expect(num(os?.total_gross)).toBe(A_PAY); expect(num(os?.total_deductions)).toBe(A_PAY * 0.1); expect(num(os?.total_net)).toBe(A_PAY - A_PAY * 0.1);
  }, 300_000);
  ...
  it.skipIf(!FIX_FILE)('closes the cross-company settlement guard without changing P1-scoped amounts semantics', async () => {
    ...
    const rest = fnProbe('REST[direct-table-ops]'); const rb = rest.before as any; expect(rb?.selectPayments).toBe(2); expect(num(rb?.updateForeign)).toBe(1); expect(num(rb?.deleteForeign)).toBe(1); expect(String(rb?.insertSpoofExpense)).toBe('inserted'); const ra = rest.after as any; expect(ra?.selectPayments).toBe(1); expect(ra?.foreignTargets?.expense ?? null).toBeNull(); expect(String(ra?.updateForeign)).toBe('no-foreign-row-visible'); expect(String(ra?.insertSpoofExpense)).toMatch(/row-level security|policy/i); expect(String(ra?.insertOwnExpense)).toBe('inserted');
    const lowRole: any = fnProbe('create_owner_settlement_draft_atomic[low-role-user]').after; expect(String(lowRole?.error ?? '')).toMatch(/ADMIN or MANAGER|42501|required/i); const anon: any = fnProbe('rpt_cash_flow[anon-execute]').after; expect(String(anon?.error ?? '')).toMatch(/permission denied/i);
    const rls = fnProbe('RLS[table-read-isolation]'); expect(rls.before).toMatchObject({ payments: 2, expenses: 2, invoices: 2, contracts: 2 }); expect(String((rls.before as any)?.owner_settlements?.error ?? '')).toMatch(/permission denied/i); expect(rls.after).toMatchObject({ payments: 1, expenses: 1, invoices: 1, contracts: 1 }); expect(String((rls.after as any)?.owner_settlements?.error ?? '')).toMatch(/permission denied/i);
  }, 300_000);

  it.skipIf(!FIX_FILE)('keeps server report numbers numerically consistent with independent SQL (parity report)', async () => {
    const truth = await directAggregates(db, COMPANY_A); const cash = outOf(fnProbe('rpt_cash_flow'), 'after'); expect(num(cash?.operating?.receipts)).toBe(truth.receipts); expect(num(cash?.operating?.expenses)).toBe(truth.expenses);
    const [openA] = await call(db, `SELECT coalesce(sum(amount + coalesce(tax_amount,0) - coalesce(paid_amount,0)),0)::numeric AS v FROM public.invoices WHERE company_id=$1 AND deleted_at IS NULL`, [COMPANY_A]); const [grossA] = await call(db, `SELECT coalesce(sum(amount),0)::numeric AS v FROM public.invoices WHERE company_id=$1 AND deleted_at IS NULL`, [COMPANY_A]); expect(num(grossA?.v)).toBe(A_PAY);
    mkdirSync(evidenceDir, { recursive: true }); writeFileSync(join(evidenceDir, 'numeric-parity.json'), JSON.stringify({ generatedAt: new Date().toISOString(), company: 'A', groundTruth: { ...truth, openReceivables: num(openA?.v) }, reports: { cash_flow: { receipts: num(cash?.operating?.receipts), expenses: num(cash?.operating?.expenses) }, vat_sales: num((fnProbe('rpt_vat_return').after as any)?.[0]?.out?.total_sales_amount) }, conclusion: 'server report outputs == independent aggregates (post-fix, company-scoped)' }, null, 2));
  }, 300_000);
});
