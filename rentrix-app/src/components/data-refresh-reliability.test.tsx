...
describe('page-by-page data refresh reliability', () => {
  it('provides an announced, retryable stale-data surface', () => {
    const html = renderToStaticMarkup(<DataRefreshAlert onRetry={() => undefined} />);
  ...
  it('keeps cached contract, property, unit, provider, and receipt details visible on refresh errors', () => {
    ...
    expect(source(files[2])).toContain("status={unit ? 'ready'");
  ...
  it('fails closed for receipt document output while the cached receipt is stale', () => {
    const receipt = source('../features/financials/receipts/receipt-detail-page.tsx');

    expect(receipt).toContain('const canUseReceiptDocument = documentSettings.isReady && !receiptQuery.isError');
    expect(receipt.match(/isReady: canUseReceiptDocument/gu)).toHaveLength(2);
    expect(receipt).toContain('disabled={isPrinting || !canUseReceiptDocument}');
  ...
  it('isolates owner ready-state hooks in a stable child component', () => {
    ...
    expect(owner).toContain("state: Extract<OwnerDetailState, { status: 'ready' }>");
    expect(owner.indexOf('function OwnerDetailReady')).toBeLessThan(owner.indexOf('const { authorization } = useAuth()'));
  });
});
