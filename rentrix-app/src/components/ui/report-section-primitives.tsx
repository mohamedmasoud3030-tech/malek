 ...
 */
export function ReportSummaryStrip({ items, className, dataReportSummary, isLoading = false }: Readonly<{ items: readonly ReportSummaryItem[]; className?: string; dataReportSummary?: string; isLoading?: boolean }>) {
  if (isLoading) {
    return (
      <div
      ...
      >
        {[70, 110, 90, 120].map((width, index) => (
          <div key={index} className="min-w-max shrink-0 px-1 py-1 sm:border-e sm:px-3 sm:py-0 sm:last:border-e-0">
            <Skeleton className="h-3 w-16" />
            <Skeleton className="mt-1 h-5 w-24" style={{ width }} />
          </div>
        ))}
      </div>
    );
