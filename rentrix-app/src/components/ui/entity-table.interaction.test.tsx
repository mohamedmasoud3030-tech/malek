...
function tableProps(
  overrides: Partial<Parameters<typeof EntityTable<Row>>[0]> = {},
) {
  return {
...
describe("EntityTable — حالات الجدول الموحد (markup)", () => {
  ...
  it("renders the shared empty state when there are no rows", () => {
    const html = renderToStaticMarkup(
      <EntityTable
        {...tableProps({
          rows: [],
          emptyTitle: "لا عناصر هنا",
          emptyDescription: "أضف أول عنصر.",
        })}
      />,
    );
  ...
  it("renders the shared loading skeleton (no rows leak through)", () => {
    const html = renderToStaticMarkup(
      <EntityTable {...tableProps({ isLoading: true })} />,
    );
  ...
  it('renders mobile cards through renderMobileCard under role="list"', () => {
    const html = renderToStaticMarkup(
      <EntityTable
        {...tableProps({
          renderMobileCard: (row: Row) => <div>بطاقة {row.name}</div>,
        })}
      />,
    );
...
describe("EntityTable — التفاعل", () => {
  ...
  it("invokes onRetry from the error state", () => {
    ...
    act(() => {
      root.render(
        <EntityTable {...tableProps({ error: new Error("boom"), onRetry })} />,
      );
    });
  ...
  it("switches to the card grid and persists the preference", () => {
    ...
    expect(window.localStorage.getItem("test:view-mode")).toBe("grid");
  ...
  it("restores a saved card preference when the table mounts", () => {
    window.localStorage.setItem("test:restored-view", "grid");
    act(() => {
      root.render(
        <EntityTable
          {...tableProps({
            enableViewModeToggle: true,
            viewModeStorageKey: "test:restored-view",
            renderMobileCard: (row: Row) => <div>بطاقة {row.name}</div>,
          })}
        />,
      );
    ...
    expect(
      container
        .querySelector('button[aria-label="عرض كبطاقات"]')
  ...
  it("does not expose a switch unless the table explicitly opts in with a card renderer", () => {
    act(() => {
      root.render(
        <EntityTable
          {...tableProps({
  ...
  it("keeps switching in memory when browser storage is unavailable", () => {
    const storageSpy = vi
      ...
      act(() => {
        root.render(
          <EntityTable
            {...tableProps({
              enableViewModeToggle: true,
              viewModeStorageKey: "test:denied-storage",
              renderMobileCard: (row: Row) => <div>بطاقة {row.name}</div>,
            })}
          />,
        );
