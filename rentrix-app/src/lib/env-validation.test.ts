...
describe('production env validation', () => {
  it('vite.config.ts contains production env guard that blocks placeholders', () => {
    ...
    expect(content).toContain('resolvePublicSupabaseConfig(url, key)');
    expect(content).toContain('config.env.VITE_SUPABASE_URL');
  ...
  it('env.ts still provides isConfigured for runtime diagnostics', () => {
    ...
    expect(content).toContain('resolvePublicSupabaseConfig');
  });
