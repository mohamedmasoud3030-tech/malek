...
function TemplateButton({ entity, format }: Readonly<{ entity: OfficeImportEntity; format: 'csv' | 'xlsx' }>) {
  ...
  return (
    <Button type="button" size="sm" variant="outline" className="min-h-10" onClick={download}>
    ...
    <div className="overflow-x-auto rounded-xl border border-border/60">
      <table className="w-full min-w-[620px] text-right text-xs">
        <thead className="bg-muted/40 text-muted-foreground">
          <tr>
            {visibleFields.map((field) => <th key={field.key} className="px-3 py-2 font-black">{field.label}</th>)}
          </tr>
        </thead>
        <tbody>
          {rows.map((row, index) => (
            <tr key={`preview-${index}`} className="border-t border-border/50">
              {visibleFields.map((field) => <td key={field.key} className="max-w-48 truncate px-3 py-2">{row[field.key] || '—'}</td>)}
            </tr>
          ))}
        </tbody>
      </table>
...
export function OfficeLaunchPanel({ draft }: Readonly<{ draft: CompanySettingsDraft }>) {
  ...
  const onFileChange = async (event: ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setPreview(buildOfficeImportPreview(entity, matrix));
    } catch (error) {
