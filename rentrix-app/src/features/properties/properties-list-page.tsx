  ...
  return (
    <>
        ...
        filters={
          <div className="space-y-2">
            <FilterBar
              ...
              onSearchChange={(value) => { ctrl.setSearch(value); ctrl.setPage(1); }}
              ...
              filters={
                <Select
                  aria-label="الحالة"
                  value={ctrl.status}
                  onChange={(e) => { ctrl.setStatus(e.target.value as typeof ctrl.status); ctrl.setPage(1); }}
                  className="w-full sm:w-36 rounded-xl"
                >
                  <option value="all">كل الحالات</option>
                  {ctrl.statusValues.map((s) => <option key={s} value={s}>{ctrl.statusLabels[s]}</option>)}
                </Select>
              }
            />
            <ActiveFilterBar filters={ctrl.activeFilters} onClearAll={ctrl.clearFilters} />
          </div>
        }
      >
        <AsyncContentState
          status={
            ctrl.propertiesQuery.isLoading ? 'loading'
            : ctrl.propertiesQuery.isError ? 'error'
            : ctrl.properties.length === 0 ? 'empty'
            : 'ready'
          }
        ...
        <DataTable
          ...
          columns={[
            ...
            { key: 'status', header: 'الحالة', render: (p) => (
              <StatusBadge tone={propertyStatusTone[p.status as keyof typeof propertyStatusTone] ?? 'gray'}>
                {ctrl.statusLabels[p.status as keyof typeof ctrl.statusLabels] ?? p.status}
              </StatusBadge>
            )},
            ...
            { key: 'actions', header: 'إجراءات', render: (p) => (
              <div className="flex" onClick={(e) => e.stopPropagation()} onKeyDown={(e) => e.stopPropagation()}>
                <ActionMenu
                  ...
                  items={[
                    { id: 'edit', label: 'تعديل', icon: Edit, onClick: () => ctrl.openEditModal(p.id) },
                    { id: 'archive', label: 'أرشفة', icon: Trash2, variant: 'destructive', onClick: () => ctrl.requestArchive(p.id, p.title ?? 'عقار') },
                  ]}
                />
              </div>
            )},
          ...
          renderMobileCard={(p) => (
            <MobileCard
              ...
              badge={<StatusBadge tone={propertyStatusTone[p.status as keyof typeof propertyStatusTone] ?? 'gray'} dot>{ctrl.statusLabels[p.status as keyof typeof ctrl.statusLabels] ?? p.status}</StatusBadge>}
        ...
        {/* Pagination */}
        {!ctrl.propertiesQuery.isLoading && !ctrl.propertiesQuery.isError && ctrl.totalPages > 1 && (
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="text-sm font-bold text-muted-foreground">{ctrl.page} / {ctrl.totalPages}</span>
            <Button
              variant="secondary"
              className="rounded-xl"
      ...
      <ConfirmDialog
        ...
        onOpenChange={(open) => { if (!open) ctrl.cancelArchive(); }}
