/**
 * Runs supabase/tests/wp03_gap005_onboarding_authority.sql against a
...
create or replace function pgtap.has_function(s text, f text, args text[] default null, name text default null) returns text
language plpgsql as $f$ begin return pgtap._rec( exists (select 1 from pg_proc p join pg_namespace n on n.oid=p.pronamespace where n.nspname = s and p.proname = f), coalesce(name,'') ); end $f$;
...
describe('WP-03 GAP-005 onboarding authority pgTAP gate (faithful shim)', () => {
  it('runs wp03_gap005_onboarding_authority.sql with zero failures', async () => {
    ...
    const stmts = splitStatements(
      transformSuite(readFileSync(join(repoRoot, 'supabase', 'tests', 'wp03_gap005_onboarding_authority.sql'), 'utf8')),
    );
