      ...
      <Card variant={rent.state === 'READY' ? 'default' : 'muted'}>
        ...
        <CardContent className="space-y-2 text-xs">
          {rent.activeProfile ? (
            <>
              <p>
                <span className="font-bold">الكود:</span> {rent.activeProfile.tax_code} — {rent.activeProfile.tax_rate}%
              </p>
              <p>
              ...
              <p>
                <span className="font-bold">الإصدار:</span> {rent.activeProfile.version_no} — {rent.activeProfile.status}
              </p>
            </>
          ) : rent.latestDraft ? (
            <>
              ...
              <Button asChild size="sm" variant="outline">
                <Link to="/settings" search={{ section: 'finance-readiness' } as never}>
                  عرض مسودات الضريبة
                </Link>
              </Button>
            </>
      ...
      <Card variant={rate.state === 'READY' ? 'default' : 'muted'}>
        ...
        <CardContent className="space-y-2 text-xs">
          {rate.activeTreatment ? (
            <>
              <p>
                <span className="font-bold">الكود:</span> {rate.activeTreatment.tax_code} — {rate.activeTreatment.tax_rate}%
              </p>
              <p>
                <span className="font-bold">سارٍ من:</span> {rate.activeTreatment.effective_from}
              </p>
              <p>
                <span className="font-bold">الإصدار:</span> {rate.activeTreatment.version_no}
              </p>
            </>
          ) : rate.latestDraft ? (
            <>
      ...
      <Card variant={fixed.state === 'READY' ? 'default' : 'muted'}>
        ...
        <CardContent className="space-y-2 text-xs">
          {fixed.activeTreatment ? (
            <>
              <p>
                <span className="font-bold">الكود:</span> {fixed.activeTreatment.tax_code} — {fixed.activeTreatment.tax_rate}%
              </p>
              <p>
                <span className="font-bold">سارٍ من:</span> {fixed.activeTreatment.effective_from}
              </p>
              <p>
                <span className="font-bold">الإصدار:</span> {fixed.activeTreatment.version_no}
              </p>
            </>
          ) : fixed.latestDraft ? (
            <>
  ...
  return (
    <div className="grid gap-3 sm:grid-cols-3">
        ...
        <CardContent className="text-xs">
          ...
          {readiness.chartOfAccounts.state !== 'READY' ? (
            <Button asChild size="sm" variant="outline" className="mt-2">
              <Link to="/reports" search={{ section: 'accounting' } as never}>
                فتح المحاسبة والرقابة
              </Link>
            </Button>
          ) : null}
        </CardContent>
      ...
      <Card>
        ...
        <CardContent className="text-xs">
          {readiness.accountingPeriod.openPeriod ? (
            <p>
              مفتوحة: {readiness.accountingPeriod.openPeriod.start_date} → {readiness.accountingPeriod.openPeriod.end_date}
            </p>
          ) : (
      ...
      <Card>
        ...
        <CardContent className="text-xs">
          {readiness.paymentMethods.state !== 'READY' ? <p className="text-destructive">تحقق من إعداد الحسابات النقدية/البنكية.</p> : <p className="flex items-center gap-1 text-success"><CheckCircle2 className="size-3.5" /> جاهز</p>}
        </CardContent>
      </Card>
    </div>
  );
