...
describe('buildAiNavigationTargets', () => {
  it('maps each declared action to the canonical in-app report/workspace', () => {
    expect(
      buildAiNavigationTargets('summarize_overdue_invoices'),
    ).toEqual([
      { label: 'فتح تعتيق المتأخرات', to: '/reports', search: { section: 'analytics', view: 'overdue' } },
    ]);
  ...
  it('never derives destinations from model output (closed union only)', () => {
    // Even an unknown action value (as if from a future/malformed payload)
    // collapses to no targets rather than navigating somewhere arbitrary.
    expect(
...
describe('isAllowedAiNavigationTarget', () => {
  ...
  it('rejects raw URLs, mutation paths, and external/contact schemes', () => {
    ...
    // Raw URLs and wa.me are rejected by the route allowlist + pattern;
    // mailto/tel rejected by pattern; unknown/mutation routes rejected by
    // the allowlist.
