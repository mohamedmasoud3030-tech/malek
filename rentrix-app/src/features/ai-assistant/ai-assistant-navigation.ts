 ...
 *
 * The assistant may navigate to the canonical product or workspace that answers the
...
import { isReportProductId } from '@/lib/report-product-ids';
...
const CONTRACTS_WORKSPACE: AiNavigationTarget = {
  label: 'فتح العقود',
  to: '/contracts',
};
...
const PROPERTIES_WORKSPACE: AiNavigationTarget = {
  label: 'فتح العقارات',
  to: '/properties',
};
const FINANCIALS_WORKSPACE: AiNavigationTarget = {
  label: 'فتح العمليات المالية',
  to: '/financials',
};
const EXPENSES_WORKSPACE: AiNavigationTarget = {
  label: 'فتح المصروفات',
  to: '/financials',
  search: { section: 'expenses', view: 'expenses' },
};
const TODAY_WORKSPACE: AiNavigationTarget = {
  label: 'فتح شاشة اليوم',
  to: '/dashboard',
};
const OWNERS_WORKSPACE: AiNavigationTarget = {
  label: 'فتح الملاك',
  to: '/owners',
};
const OWNER_SETTLEMENTS: AiNavigationTarget = {
...
function entityDetailTarget(
  surface: AiAssistantSurfaceContext,
): AiNavigationTarget | null {
  const id = surface.entityId;
  if (!id || !SAFE_ID_PATTERN.test(id)) return null;
  switch (surface.entityType) {
    case 'property':
      return { label: 'فتح ملف العقار', to: `/properties/${id}` };
    case 'contract':
      return { label: 'فتح العقد', to: `/contracts/${id}` };
    case 'tenant':
      return { label: 'فتح ملف المستأجر', to: `/tenants/${id}` };
    case 'owner':
      return { label: 'فتح ملف المالك', to: `/owners/${id}` };
    case 'person':
      return { label: 'فتح ملف الشخص', to: `/people/${id}` };
    default:
      return null;
  }
}
...
function contextualTargets(
  ...
  switch (surface.entityType) {
    case 'property':
      return [detail, MAINTENANCE_WORKSPACE, OVERDUE_REPORT].filter(
        Boolean,
      ) as AiNavigationTarget[];
    case 'unit':
      return [CONTRACTS_WORKSPACE, MAINTENANCE_WORKSPACE];
    case 'contract':
      return [detail, OVERDUE_REPORT, COMMUNICATION_CENTER].filter(
        Boolean,
      ) as AiNavigationTarget[];
    case 'tenant':
    case 'person':
    ...
    case 'owner':
      return [detail, OWNER_SETTLEMENTS, OFFICE_PERFORMANCE].filter(
        Boolean,
      ) as AiNavigationTarget[];
    default:
      return [];
...
function dedupeTargets(
  targets: readonly AiNavigationTarget[],
): readonly AiNavigationTarget[] {
...
export function buildAiNavigationTargets(
  ...
  const targets =
    action === 'explain_current_surface'
      ...
      : baseTargets;
  return dedupeTargets(targets).filter((target) =>
    isSafeNavigationRoute(target.to),
  );
}
...
export function isAllowedAiNavigationTarget(
  target: AiNavigationTarget,
): boolean {
  if (!isSafeNavigationRoute(target.to)) return false;
  const serialized = JSON.stringify(target);
  return FORBIDDEN_NAVIGATION_PATTERNS.every(
