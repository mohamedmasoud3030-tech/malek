  ...
  return (
    <>
        ...
        headerExtra={
          form.formState.isDirty && !saveMutation.isPending ? (
            <StatusBadge tone="warning">
              {translateSharedLabel(
                "unsavedChanges",
                getAppLanguageState().language,
              )}
            </StatusBadge>
          ) : undefined
        }
        ...
        {isProviderLoading ? (
          ...
          >
            ...
            <EntityForm.Section
              ...
              description="الاسم التشغيلي مطلوب؛ أما بيانات التسجيل والضريبة فتسجل عند توفرها."
            >
            ...
            <EntityForm.Section
              ...
              description="جهة الاتصال والقنوات التي يستخدمها فريق التشغيل للتنسيق."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <EntityForm.Field
            ...
            <EntityForm.Section
              title="المعلومات التشغيلية"
              description="نطاق التغطية وملاحظات التوفر بدون افتراض أسعار أو شروط دفع."
            >
              <div className="grid gap-4 sm:grid-cols-2">
                <EntityForm.Field
                  label="نطاق الخدمة"
                ...
                >
                  <Textarea
                    ...
                    placeholder="مثال: مسقط والمناطق القريبة"
                    {...form.register("service_area")}
                  />
                </EntityForm.Field>
                <EntityForm.Field
                  label="ملاحظات التوفر"
                  error={form.formState.errors.availability_notes?.message}
                >
                  <Textarea
                    aria-label="ملاحظات التوفر"
                    placeholder="أوقات التواصل أو تغطية الطوارئ الموثقة"
              ...
              >
                <Textarea
                  aria-label="ملاحظات مزود الخدمة"
                  className="min-h-24"
                  {...form.register("notes")}
                />
              </EntityForm.Field>
            ...
            <EntityForm.Section
              title="أنواع الخدمات المدعومة"
              description="اختر الأنواع المعرفة في سجل مزودي الخدمات؛ يمكن تعديلها من إدارة أنواع الخدمات."
            >
              ...
              (categoriesQuery.data ?? []).length === 0 ? (
                <p className="rounded-xl border border-dashed p-4 text-sm text-muted-foreground">
                  لا توجد أنواع خدمات معرفة بعد. يمكن حفظ المزود ثم تعريف
                  الأنواع وربطها لاحقًا.
                </p>
              ) : null}
              <Controller
                ...
                render={({ field }) => (
                  <div
                    ...
                    aria-label="أنواع الخدمات المدعومة"
                  >
                    {(categoriesQuery.data ?? []).map((category) => {
                      ...
                      return (
                        <label
                          key={category.id}
                          className="flex min-h-11 items-start gap-3 rounded-xl border border-border bg-muted/20 p-3 text-sm"
                        >
                          <input
                            type="checkbox"
                            checked={checked}
      ...
      <ConfirmDialog
        ...
        onOpenChange={(next) => {
          if (!next && !saveMutation.isPending) setShowDiscardDialog(false);
        }}
