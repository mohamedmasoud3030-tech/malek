...
describe('Service Providers database contract', () => {
  it('creates one canonical provider table and normalized maintainable categories', () => {
    ...
    expect(migration).toContain('create table public.service_provider_categories');
    expect(migration).toContain('create table public.service_provider_category_links');
  ...
  it('uses the active-company and semantic-permission architecture on every provider table', () => {
    ...
    expect(migration).toContain("current_user_has_effective_app_permission('service_providers.view')");
    expect(migration).toContain("current_user_has_effective_app_permission('service_providers.write')");
    expect(migration).toContain('company_id = public.current_company_id()');
    expect(migration).toContain("('service_providers.view', 'عرض مزودي الخدمات'");
    expect(migration).toContain("('service_providers.write', 'إضافة وتعديل وأرشفة مزودي الخدمات'");
  });
  ...
  it('makes Maintenance assignments company-safe and capability-safe', () => {
    ...
    expect(migration).toContain('foreign key (service_provider_id, company_id)');
    expect(migration).toContain('maintenance_service_provider_category_company_fk');
    expect(migration).toContain('validate_maintenance_service_provider_assignment');
    expect(migration).toContain('p_service_provider_category_id uuid default null');
  ...
  it('extends the existing document platform instead of creating provider attachments', () => {
    ...
    expect(migration).not.toContain('create table public.service_provider_documents');
    expect(migration).not.toContain('create table public.service_provider_attachments');
  });
  ...
  it('ships a manual rollback that restores the prior Maintenance RPC and document contract', () => {
    ...
    expect(rollback).toContain('drop table if exists public.service_provider_category_links');
    expect(rollback).toContain('drop column if exists service_provider_id');
    expect(rollback).toContain('create function public.create_maintenance_atomic');
