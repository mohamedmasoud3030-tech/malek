...
export const OFFICE_IMPORT_MAX_COLUMNS = 64;
const OFFICE_IMPORT_MAX_ZIP_ENTRIES = 256;
const OFFICE_IMPORT_MAX_UNCOMPRESSED_ENTRY_BYTES = 8 * 1024 * 1024;

function assertMatrixBounds(matrix: readonly (readonly string[])[]) {
  if (matrix.length > OFFICE_IMPORT_MAX_ROWS + 1) {
    throw new Error(`الملف يتجاوز الحد المسموح: ${OFFICE_IMPORT_MAX_ROWS} سجل كحد أقصى`);
  }
  if (matrix.some((row) => row.length > OFFICE_IMPORT_MAX_COLUMNS)) {
    throw new Error(`الملف يتجاوز الحد المسموح: ${OFFICE_IMPORT_MAX_COLUMNS} عموداً كحد أقصى`);
  }
}
...
function readZipDirectory(bytes: Uint8Array): ZipDirectoryEntry[] {
  ...
  if (entryCount > OFFICE_IMPORT_MAX_ZIP_ENTRIES) {
    throw new Error('ملف Excel يحتوي على عدد مفرط من مكونات ZIP');
  }
...
async function unzipEntry(bytes: Uint8Array, entry: ZipDirectoryEntry): Promise<Uint8Array> {
  if (entry.size > OFFICE_IMPORT_MAX_UNCOMPRESSED_ENTRY_BYTES) {
    throw new Error('ملف Excel يحتوي على جزء غير مضغوط أكبر من الحد الآمن');
  }
  const view = new DataView(bytes.buffer, bytes.byteOffset, bytes.byteLength);
...
function restoreNeutralizedSpreadsheetText(value: string) {
...
function parseWorksheet(source: string, sharedStrings: readonly string[]): string[][] {
  ...
  for (const rowNode of Array.from(doc.querySelectorAll('sheetData > row'))) {
    ...
    for (const cell of Array.from(rowNode.querySelectorAll('c'))) {
      ...
      row[index] = restoreNeutralizedSpreadsheetText(value);
    }
...
export async function parseXlsxMatrix(bytes: Uint8Array): Promise<string[][]> {
  ...
  const matrix = parseWorksheet(sheetSource, sharedStrings);
  assertMatrixBounds(matrix);
  return matrix;
}
