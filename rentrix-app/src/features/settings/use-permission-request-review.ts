...
import { decidePermissionRequest, listPermissionRequestsForReview, revokePermissionGrant, type PermissionRequest } from '@/features/auth/permission-request-service';
...
export function usePermissionRequestReview() {
  ...
  const decisionMutation = useMutation({
    ...
    onSuccess: async (_result, variables) => { await refresh(); setRejecting(null); setDecisionReason(''); toast.success(variables.decision === 'APPROVED' ? 'تم اعتماد الصلاحية وتفعيلها.' : 'تم رفض الطلب وتسجيل السبب.'); },
  ...
  return { requestsQuery, requests: requestsQuery.data ?? [], decisionMutation, revokeMutation, rejecting, setRejecting, decisionReason, setDecisionReason };
}
