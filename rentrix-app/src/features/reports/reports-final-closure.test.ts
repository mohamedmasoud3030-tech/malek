...
import { REPORT_WORKSPACES } from './report-workspaces';
import { reportGroups, businessReportGroups, specialistReportGroups } from './directory/report-directory-groups';
import { ANALYTICS_REPORT_VIEWS, ACCOUNTING_REPORT_VIEWS } from './report-view-registry';
import { previousPeriodRange, formatPointChange, formatSignedAmountChange } from './documents/report-period';
import {
...
import { buildPropertyReportPayload, type PropertyReportData } from './documents/professional-property-report';
...
describe('reports closure — financial semantics', () => {
  ...
  it('keeps the three-way occupancy denominator (occupied + vacant + nonRentable)', () => {
    const scope = buildPropertyAnalyticsScope([
      { property: 'p', propertyId: 'p', shortPropertyId: '', hasTitle: true, occupied: 5, vacant: 3, nonRentable: 2 },
    ]);
  ...
  it('never publishes zero for an unavailable metric', () => {
    ...
    const executive = buildPropertyAnalyticsExecutive({ occupancyRows: [], expenseRows: [], performanceRows: [] });
    for (const value of [executive.collected, executive.due, executive.overdue, executive.expenses, executive.expensePerOccupiedUnit, executive.scope.occupancyRate]) {
  ...
  it('keeps the priority score an operational ordering, never a risk probability', async () => {
    ...
    const { buildPropertyAnalyticsInsights } = await import('./property-analytics-model');
    const insights = buildPropertyAnalyticsInsights({
      occupancyRows: [{ property: 'p', propertyId: 'p', shortPropertyId: '', hasTitle: true, occupied: 1, vacant: 4, nonRentable: 0 }],
      expenseRows: [],
      performanceRows: [],
      openMaintenanceCount: 6,
      expiringContractsCount: 3,
      longestVacancyDays: 120,
      previous: { from: '2026-01-01', to: '2026-01-31', occupancyRate: 90, due: null, collected: null, overdue: null, expenses: null },
    });
...
describe('reports closure — current vs previous comparable period', () => {
  it('uses one deterministic definition of the previous period', () => {
    ...
    expect(previousPeriodRange('2026-02-10', '2026-02-19')).toEqual({ from: '2026-01-31', to: '2026-02-09' });
  ...
  it('omits the comparison entirely when the previous period is unavailable', () => {
    expect(buildPropertyAnalyticsComparison({ occupancyRows: [], expenseRows: [], performanceRows: [] })).toEqual([]);
  });
...
describe('reports closure — presentation contracts', () => {
  it('routes report drill-through only through the canonical ReportDrillAction', () => {
    ...
    for (const surface of surfaces) {
      ...
      expect(source, `${surface} must use ReportDrillAction`).toContain('ReportDrillAction');
      expect(source, `${surface} must not hand-roll a drill button`).not.toMatch(/<button[\s>]/);
    }
  });

  it('keeps exactly one shared segmented switcher for report bodies', () => {
    const primitives = readFileSync(resolve(srcDir, 'components/ui/report-section-primitives.tsx'), 'utf8');
    expect(primitives).toContain('export function ReportSegmentedTabs');
    for (const surface of ['components/AccountingReportsSection.tsx', 'components/GeneralLedgerCoreSection.tsx']) {
      const source = read(surface);
      expect(source, `${surface} must compose ReportSegmentedTabs`).toContain('<ReportSegmentedTabs');
      expect(source, `${surface} must not re-implement the segmented track`).not.toContain('rounded-xl border border-border/60 bg-muted/20 p-1');
    }
  });

  it('keeps one shell header hierarchy — report bodies never render a second page header', () => {
    for (const file of ['components/PropertyAnalyticsSection.tsx', 'components/OverviewSection.tsx', 'components/CollectionsSection.tsx', 'components/StatementsSection.tsx']) {
      expect(read(file), `${file} must not render a PageHeader`).not.toContain('<PageHeader');
    }
  ...
  it('keeps report output actions behind the shared control', () => {
    ...
    expect((section.match(/<ReportOutputActions/g) ?? []).length).toBe(1);
  });
  ...
  it('does not keep a second, unreachable report directory surface', () => {
    ...
    expect(read('reports-page.tsx')).toContain('<ReportsPrimaryNavigation');
  });
});
...
describe('reports closure — Reports Center reachability', () => {
  it('gives every catalogue entry exactly one owning workspace and no duplicate titles', () => {
    const titles = reportGroups.map((group) => group.title);
    expect(new Set(titles).size).toBe(titles.length);
    const descriptions = reportGroups.map((group) => group.description);
    expect(new Set(descriptions).size).toBe(descriptions.length);
    expect(businessReportGroups).toHaveLength(6);
    expect(specialistReportGroups).toHaveLength(1);
  });

  it('makes every registered analytics view reachable from a workspace', () => {
    const reachable = new Set(REPORT_WORKSPACES.flatMap((workspace) => [
      workspace.defaultView,
      ...workspace.subViews.map((subView) => subView.id),
      ...workspace.legacyViews,
    ]));
    for (const view of ANALYTICS_REPORT_VIEWS) {
      expect(reachable.has(view.id), `analytics view ${view.id} must be reachable`).toBe(true);
    }
    for (const view of ACCOUNTING_REPORT_VIEWS) {
      expect(reachable.has(view.id), `accounting view ${view.id} must be reachable`).toBe(true);
    }
  });

  it('uses business language, never developer terminology, in the catalogue', () => {
    for (const group of reportGroups) {
      for (const text of [group.title, group.description, ...group.shortcuts.flatMap((s) => [s.label, s.description])]) {
        expect(text).not.toMatch(/RPC|adapter|read model|payload|registry|snapshot/i);
      }
    }
  });
});
...
describe('reports closure — document outputs', () => {
  ...
  it('registers the Golden Report and statement outputs Reports depends on', () => {
    for (const type of ['owner_report', 'property_report', 'tenant_statement', 'owner_statement', 'generic_report']) {
      expect(registeredTypes.has(type as never), `${type} must be registered`).toBe(true);
    }
  });

  it('keeps every Reports-owned document on A4 with print and PDF outputs', () => {
    for (const type of ['owner_report', 'property_report', 'tenant_statement']) {
  ...
  it('renders an unavailable amount as an em dash, never as a printed zero', () => {
    const data: PropertyReportData = {
      ...
      asOf: '2026-02-28',
      occupancy: { units: 0, occupied: 0, vacant: 0, nonRentable: 0, rate: null },
      summary: null,
      overdueTotal: null,
      arrearsAsOf: null,
      arrearsBuckets: null,
    ...
    const kpis = payload.groups[0].blocks.find((block) => block.kind === 'kpis');
    expect(kpis).toBeDefined();
    const occupancyKpi = kpis && kpis.kind === 'kpis' ? kpis.kpis.find((kpi) => kpi.label === 'نسبة الإشغال') : undefined;
    expect(occupancyKpi?.value).toEqual({ kind: 'text', value: '—' });
    const collectedKpi = kpis && kpis.kind === 'kpis' ? kpis.kpis.find((kpi) => kpi.label === 'المحصل للفترة') : undefined;
  ...
  it('omits the comparison block truthfully when there is no previous period', () => {
    const data = {
      propertyTitle: 'برج الخوير', scopeLabel: 'العقار: برج الخوير', periodFrom: '2026-02-01', periodTo: '2026-02-28',
      generatedAt: '2026-03-01', isPortfolioScope: false, collectionRateCurrent: null, collectionRatePrevious: null,
      asOf: '2026-02-28', occupancy: { units: 10, occupied: 8, vacant: 1, nonRentable: 1, rate: 80 },
      summary: null, overdueTotal: 100, arrearsAsOf: null, arrearsBuckets: null, expenseTotal: 50,
      expenseByCategory: [], maintenancePeriodCount: 0, maintenanceOpenAsOf: 0, expiringCount: 0,
      expiringRentExposed: null, expiringRowsSlice: null, longestVacancyDays: null, averageVacancyDays: null,
      vacancyCount: 0, vacancyRows: null, monthlyCollectionTrend: [], occupancyTrend: [], unitRows: [],
      previous: null, portfolio: null,
    } satisfies PropertyReportData;
    const payload = buildPropertyReportPayload(data);
    const blocks = payload.groups[0].blocks;
    const comparisonTable = blocks.find((block) => block.kind === 'table' && (block.table.title ?? '').includes('مقارنة الأداء'));
    expect(comparisonTable).toBeUndefined();
    const note = blocks.find((block) => block.kind === 'note');
    expect(note && note.kind === 'note' ? note.note.text : '').toContain('لا تتوفر فترة سابقة قابلة للمقارنة');
  });
});
