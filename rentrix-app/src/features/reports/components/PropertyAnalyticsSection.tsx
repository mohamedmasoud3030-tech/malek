...
function changeLabel(row: PropertyAnalyticsComparisonRow): string {
  ...
  if (row.kind === 'count') return `${sign}${formatLatinNumber(row.change, 'ar')}`;
  return `${sign}${formatMoney(row.change)}`;
}

function changeTone(row: PropertyAnalyticsComparisonRow): 'success' | 'danger' | 'neutral' {
  if (row.change == null || row.direction === 'flat') return 'neutral';
  const improved = row.higherIsBetter ? row.direction === 'up' : row.direction === 'down';
  return improved ? 'success' : 'danger';
}

function metricValue(value: MetricValue, kind: PropertyAnalyticsComparisonRow['kind']): string {
...
export function PropertyAnalyticsSection({
  ...
  const comparisonRows = comparison ?? buildPropertyAnalyticsComparison(fallbackInput);
  const benchmarkRows = benchmark ?? buildPropertyAnalyticsBenchmark(fallbackInput);
  ...
  const highestExpenseShare = highestExpenseProperty ? rateOf(highestExpenseProperty.total, totalExpenses) : null;
  ...
  return (
    <div className="space-y-3">
      <ReportSummaryStrip
        ...
        items={[
          { label: 'العقارات في النطاق', value: metricCount(scope.properties), detail: `${metricCount(scope.units)} وحدة` },
          {
            label: 'نسبة الإشغال',
            value: metricRate(scope.occupancyRate),
            detail: `${metricCount(scope.occupied)} مشغولة`,
            tone: scope.occupancyRate != null && scope.occupancyRate < 75 ? 'warning' : undefined,
          },
          { label: 'وحدات شاغرة', value: metricCount(scope.vacant), detail: `${metricCount(scope.nonRentable)} غير قابلة للتأجير` },
          { label: 'محصّل الفترة', value: metricMoney(summary.collected), detail: `مستحق ${metricMoney(summary.due)}` },
          { label: 'المتأخرات', value: metricMoney(summary.overdue), detail: 'كما في تاريخ الإعداد', tone: summary.overdue != null && summary.overdue > 0 ? 'critical' : undefined },
          { label: 'المصروفات المسجلة', value: metricMoney(summary.expenses), detail: `${metricMoney(summary.expensePerOccupiedUnit)} للوحدة المشغولة` },
          { label: 'صيانة مفتوحة', value: metricCount(summary.openMaintenance), detail: `${metricCount(summary.expiringContracts)} عقد ينتهي قريبًا` },
          {
            label: 'قيمة إيجار مرجعية للشواغر',
            value: metricMoney(summary.vacancyReferenceRent),
            detail: summary.longestVacancyDays != null ? `أطول شغور ${metricCount(summary.longestVacancyDays)} يوم` : 'لا شواغر قائمة',
          },
        ]}
      />
          ...
          <ReportList>
            {comparisonRows.map((row) => (
              <ReportListRow
                ...
                value={<StatusBadge tone={changeTone(row)}>{changeLabel(row)}</StatusBadge>}
              />
            ))}
          </ReportList>
        </ReportPanel>
          ...
          <ReportList>
            {benchmarkRows.map((row) => (
              <ReportListRow
                ...
                value={<span dir="ltr">{metricValue(row.portfolio, row.kind)}</span>}
      ...
      <div className="grid gap-3 sm:grid-cols-2">
        {scope.occupancyRate != null ? (
          <ReportProgress
            ...
            tone={scope.occupancyRate >= 90 ? 'good' : scope.occupancyRate >= 75 ? 'warning' : 'critical'}
          />
        ) : null}
      ...
      <ReportInsightNote title="ما الذي يحتاج انتباهًا؟">
        <ul className="list-inside list-disc space-y-1">
          {insightRows.map((insight) => (
            <li key={insight.key} data-insight-tone={insight.tone}>{insight.text}</li>
          ))}
        </ul>
        <p className="mt-2 text-xs">
          أولوية المتابعة ترتيب تشغيلي محسوب من ضغط المتأخرات والشغور والصيانة المفتوحة وعبء المصروفات؛ ليست احتمال تعثر ولا بديلًا عن القوائم المحاسبية.
          تكلفة الصيانة غير المرحّلة ضغط تشغيلي وليست مصروفًا ماليًا. قيمة الإيجار المرجعية للشواغر ليست إيرادًا ولا ذمة مدينة.
        </p>
      </ReportInsightNote>
        ...
        {performanceRows.length === 0 && occupancyRows.length === 0 ? (
          ...
          <ReportList>
            {performanceRows.map((row) => {
              ...
              const rowUnits = row.occupiedUnits + row.vacantUnits + row.nonRentableUnits;
                ...
                <ReportListRow
                  ...
                  value={(
                    <div className="space-y-1 text-end">
                      ...
                      <p className="text-xs font-medium text-muted-foreground" dir="ltr">أولوية {row.riskScore}/100 · {metricRate(rowOccupancy)} · مصروفات {formatMoney(row.expenses)}</p>
                    </div>
                  )}
                  action={(
          ...
          <ReportList>
            {occupancyRows.map((row) => {
              ...
              const propertyExpensePerOccupied = perUnit(expense?.total ?? null, row.occupied);
              return (
                <ReportListRow
                  key={row.propertyId}
                  title={row.property}
                  ...
                  value={(<div className="text-end"><p dir="ltr">{metricRate(rate)}</p><p className="mt-1 text-xs font-medium text-muted-foreground" dir="ltr">{expense ? formatMoney(expense.total) : UNAVAILABLE}</p></div>)}
                  action={(
                    <div className="flex flex-wrap gap-1">
                      <ReportDrillAction
                        label="الشواغر"
