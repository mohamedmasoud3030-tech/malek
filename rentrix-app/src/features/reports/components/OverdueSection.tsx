...
export function OverdueSection({ rows, agedReport, summary, canExportReports, isLoading }: Readonly<{
  ...
  const over90Share = typeof totalOverdue === 'number' && totalOverdue > 0 && typeof over90Amount === 'number'
  ...
  const topExposureShare = topExposure && typeof totalOverdue === 'number' && totalOverdue > 0
  ...
  const tableRemainingTotal = rows.reduce((total, row) => total + row.remainingAmount, 0);
    ...
    sections: [
      {
        ...
        totals: ['إجمالي جدول الفواتير', '', '', '', '', '', '', '', '', `${formatLatinNumber(tableRemainingTotal, 'ar-OM')} ${currencySymbol}`, '', ''],
      },
    ],
  ...
  return (
    <div className="space-y-3">
        ...
        items={[
          {
            ...
            value: hasArrearsSummary ? formatMoney(totalOverdue) : UNAVAILABLE_VALUE,
            detail: hasArrearsSummary ? 'لا يشمل الرصيد الجاري' : UNAVAILABLE_DETAIL,
          },
          {
            label: 'الفواتير المتأخرة',
            value: hasArrearsSummary ? formatLatinNumber(overdueInvoiceCount ?? 0, 'ar') : UNAVAILABLE_VALUE,
            detail: hasArrearsSummary ? 'فواتير مفتوحة' : UNAVAILABLE_DETAIL,
          },
          {
            label: 'متوسط التأخير',
            value: hasArrearsSummary ? `${formatLatinNumber(Math.round(averageDelay ?? 0), 'ar')} يوم` : UNAVAILABLE_VALUE,
            detail: hasArrearsSummary ? 'متوسط عمر المتأخر' : UNAVAILABLE_DETAIL,
          },
          {
            label: 'أكثر من 90 يوم',
            value: hasArrearsSummary ? formatMoney(over90Amount) : UNAVAILABLE_VALUE,
            detail: hasArrearsSummary ? `${formatLatinNumber(over90Count ?? 0, 'ar')} عالية المخاطر` : UNAVAILABLE_DETAIL,
            tone: hasArrearsSummary && over90Share > 40 ? 'critical' : undefined,
          },
        ]}
      />
      ...
      <ReportColumns>
        <AgingBucketsPanel rows={bucketRows} currentBucketLabel={currentBucketLabel} action={agingAction} isLoading={isLoading} />
        <div className="space-y-3">
          {hasArrearsSummary ? (
