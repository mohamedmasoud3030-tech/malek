...
function contextFor(role: AuthorizationRole): AuthorizationContext {
  return {
    userId: `user-${role}`,
    email: `${role.toLowerCase()}@malek.test`,
    role,
  };
}

type MatrixRow = { permission: AppPermission } & Record<
  AuthorizationRole,
  boolean
>;
...
const financialMatrix: MatrixRow[] = [
  ...
  {
    permission: 'financial.fixed_monthly_accruals.view',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.invoices.export',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.receipts.void',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: false,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.reports.export',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.bank_reconciliation.match',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.owner_settlements.approve',
    ADMIN: true,
    MANAGER: false,
    ACCOUNTANT: false,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.owner_settlements.pay',
    ADMIN: true,
    MANAGER: false,
    ACCOUNTANT: false,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.fixed_monthly_accruals.execute',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
  ...
  {
    permission: 'financial.fixed_monthly_accruals.reverse',
    ADMIN: true,
    MANAGER: true,
    ACCOUNTANT: true,
    OPERATIONS: false,
...
describe('R5 — view/action separation invariants', () => {
  ...
  it('the reports view capability exists as its own catalog entry with a label', () => {
    ...
    expect(getPermissionLabel('financial.reports.view')).toBe(
      'عرض التقارير المالية',
    );
    expect(financialOperationPermissions.viewReports).toBe(
      'financial.reports.view',
    );
    expect(financialOperationPermissions.exportReports).toBe(
...
describe('R5 — route/navigation vocabulary alignment', () => {
  it('the /reports route and nav item gate on VIEW, export stays an action', async () => {
    ...
    const routeTree = readFileSync(
      resolve(process.cwd(), 'src/app/router/route-tree.ts'),
      'utf8',
    );
    const navItems = readFileSync(
      resolve(process.cwd(), 'src/app/navigation/app-nav-items.ts'),
      'utf8',
    );
    const reportsPage = readFileSync(
      resolve(process.cwd(), 'src/features/reports/reports-page.tsx'),
      'utf8',
    );
    const reportProductPage = readFileSync(
      resolve(
        process.cwd(),
    ...
    // The catalog permits viewing only. The canonical product renderer owns
    // export actions and keeps that separate permission visible at the point
    // where a document/output is actually available.
    expect(reportsPage).toContain('financialOperationPermissions.viewReports');
    expect(reportProductPage).toContain(
