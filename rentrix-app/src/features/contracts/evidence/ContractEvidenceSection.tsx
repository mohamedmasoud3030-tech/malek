      ...
      <div className="grid gap-4 xl:grid-cols-2">
        <Card>
          ...
          <CardContent className="space-y-4">
            {!profile ? <div className="rounded-xl border border-warning/30 bg-warning/5 p-3"><p className="font-bold text-warning">غير مهيأ قانونياً</p><p className="mt-1 text-xs leading-5 text-muted-foreground">لن يخترع MALEK جهة أو مهلة أو رسماً. يلزم ملف معتمد ونافذ قبل بدء التسجيل.</p></div> : null}
            {profile && !profile.registration_required ? <StatusBadge tone="neutral">غير مطلوب وفق الملف المعتمد</StatusBadge> : null}
            {profile && profile.registration_required && registration ? <div className="space-y-2"><StatusBadge tone={registrationTone(registration.status)}>{registrationStatusLabels[registration.status] ?? registration.status}</StatusBadge><p className="text-sm">تاريخ التقديم: <span dir="ltr">{registration.submitted_on}</span></p>{registration.registration_reference ? <p className="text-sm">مرجع التسجيل: <strong dir="ltr">{registration.registration_reference}</strong></p> : null}{registration.decision_reason ? <p className="text-sm text-danger">السبب: {registration.decision_reason}</p> : null}{registration.status === 'SUBMITTED' && canVerify ? <Button variant="secondary" onClick={() => { setFormError(null); setDecisionOpen(true); }}>التحقق من الطلب</Button> : null}</div> : null}
            {profile && profile.registration_required && !registration ? <div className="space-y-3"><p className="text-sm text-muted-foreground">التسجيل مطلوب وفق ملف الشركة المعتمد، ولم يُسجل طلب بعد.</p>{canOperate ? <Button onClick={() => { setFormError(null); setRegistrationOpen(true); }}>بدء طلب التسجيل</Button> : null}</div> : null}
          </CardContent>
        </Card>
