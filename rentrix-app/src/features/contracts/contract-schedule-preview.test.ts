...
describe('calculateContractSchedulePreview canonical duration contract', () => {
  it('calculates exact monthly installment counts without dividing the contractual per-cycle rent amount', () => {
    const months6 = calculateContractSchedulePreview('2026-01-01', '2026-06-30', 'monthly', 600);
    expect(months6.installmentCount).toBe(6);
    expect(months6.amountPerInstallment).toBe(600);
    ...
    expect(months12.amountPerInstallment).toBe(1200);
    ...
    expect(months24.amountPerInstallment).toBe(2400);
  });

  it('calculates exact quarterly and semi-annual cycle counts while preserving the per-cycle payment amount', () => {
    const q1Year = calculateContractSchedulePreview('2026-01-01', '2026-12-31', 'quarterly', 1200);
    expect(q1Year.installmentCount).toBe(4);
    expect(q1Year.amountPerInstallment).toBe(1200);
    ...
    expect(s2Years.amountPerInstallment).toBe(2000);
  });
