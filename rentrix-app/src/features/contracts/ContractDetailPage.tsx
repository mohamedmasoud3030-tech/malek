...
export function ContractDetailPage() {
  ...
  return (
    <PageLayout dir="rtl" size="wide">
      ...
      <Card className="overflow-hidden border-primary/20 bg-primary/5"><CardHeader className="bg-background/80"><CardTitle className="flex items-center gap-2"><ShieldAlert className="size-5 text-primary" />إجراءات التجديد والإنهاء</CardTitle><CardDescription>{getExpiryDescription(companySettings, contract)}</CardDescription></CardHeader><CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6"><p className="text-sm text-muted-foreground">{contract.renewed_from ? 'هذا العقد مجدد من عقد سابق.' : 'لا يوجد عقد سابق مرتبط بهذا العقد.'}</p><div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">{renewalAllowed ? <Button variant="secondary" className="min-h-11" onClick={() => { form.reset(getRenewalDefaults(contract)); setOpen(true); }}><RefreshCw className="me-2 size-4" />تجديد العقد</Button> : null}{canTerminateContract(contract) ? <Button variant="destructive" className="min-h-11" onClick={() => { setTerminateReason(''); setTerminateOpen(true); }}>إنهاء العقد بسبب</Button> : <Button variant="secondary" className="min-h-11" asChild><Link to="/contracts/$contractId/edit" params={{ contractId: contract.id }}>تعديل الحالة وسبب الإلغاء</Link></Button>}</div></CardContent></Card>
