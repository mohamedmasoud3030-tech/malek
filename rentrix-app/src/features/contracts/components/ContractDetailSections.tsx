...
export function ContractLifecycleSection({ contract, settings, renewalAllowed, onRenew, canTerminate, onTerminate }: Readonly<{ contract: ContractDetail; settings: CompanySettingsContract; renewalAllowed: boolean; onRenew: () => void; canTerminate: boolean; onTerminate: () => void }>) {
  return <Card className="overflow-hidden border-primary/20 bg-primary/5"><CardHeader className="bg-background/80"><CardTitle className="flex items-center gap-2"><ShieldAlert className="size-5 text-primary" />إجراءات التجديد والإنهاء</CardTitle><CardDescription>{getExpiryDescription(settings, contract)}</CardDescription></CardHeader><CardContent className="flex flex-wrap items-center justify-between gap-3 pt-6"><p className="text-sm text-muted-foreground">{contract.renewed_from ? 'هذا العقد مجدد من عقد سابق.' : 'لا يوجد عقد سابق مرتبط بهذا العقد.'}</p><div className="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">{renewalAllowed ? <Button variant="secondary" className="min-h-10" onClick={onRenew}>تجديد العقد</Button> : null}{canTerminate ? <Button variant="destructive" className="min-h-10" onClick={onTerminate}>إنهاء العقد بسبب</Button> : <Button variant="secondary" className="min-h-10" asChild><Link to="/contracts/$contractId/edit" params={{ contractId: contract.id }}>تعديل الحالة وسبب الإلغاء</Link></Button>}</div></CardContent></Card>;
}
