...
export function ContractOverviewSection({ contract, settings }: Readonly<{ contract: ContractDetail; settings: CompanySettingsContract }>) {
  return <Card><CardHeader><CardTitle>بيانات العقد</CardTitle><CardDescription>الحقول الأساسية وربط العقار والوحدة والمستأجر.</CardDescription></CardHeader><CardContent><DetailFields fields={[{ label: 'العقد رقم', value: contract.reference ?? 'عقد بلا مرجع تجاري' }, { label: 'المستأجر', value: contract.people?.full_name }, { label: 'الوحدة', value: contract.units?.unit_number }, { label: 'العقار', value: contract.properties?.title }, { label: 'تاريخ البداية', value: formatContractDate(settings, contract.start_date) }, { label: 'تاريخ النهاية', value: formatContractDate(settings, contract.end_date) }, { label: 'قيمة الإيجار', value: formatContractMoney(settings, contract.rent_amount) }, { label: 'دورة السداد', value: paymentCycleLabels[contract.payment_cycle] }, { label: 'الحالة', value: <StatusBadge tone={contractStatusTone[normalizeContractStatus(contract.status)]}>{contractStatusLabels[normalizeContractStatus(contract.status)]}</StatusBadge> }, { label: 'اتفاقية الإدارة', value: contract.agreement_id ? 'اتفاقية إدارة مرتبطة' : 'لا توجد اتفاقية مرتبطة' }, { label: 'سبب الإلغاء', value: isContractStatus(contract.status, 'terminated') ? contract.cancellation_reason?.trim() || '—' : 'غير مطبق' }, { label: 'ملاحظات', value: contract.notes, wide: true }]} /></CardContent></Card>;
}
