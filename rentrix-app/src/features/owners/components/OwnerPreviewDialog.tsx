...
export function OwnerPreviewDialog({
  ...
  const firstProperties = (row?.properties ?? []).slice(0, 2);
                  ...
                  value: (
                    <span className="flex flex-wrap gap-1.5">
                      {firstProperties.map((property) => (
                        <span key={property.id} className="rounded-lg bg-muted/50 px-2 py-1 text-xs font-medium">
                          {property.title ?? 'عقار بلا عنوان'}
                        </span>
                      ))}
                      {extraProperties > 0 ? (
