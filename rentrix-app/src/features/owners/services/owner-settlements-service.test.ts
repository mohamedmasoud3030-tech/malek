...
describe('summarizeLiveOwnerSettlements', () => {
  it('sums gross/fees/deductions/net across live settlements', () => {
    ...
    expect(totals).toEqual({ gross: 1500, fees: 150, expenses: 20, feeVat: 5, net: 1325 });
  });
  ...
  it('excludes cancelled drafts from every total — they never create a payable or collection', () => {
    ...
    expect(totals).toEqual({ gross: 1000, fees: 100, expenses: 20, feeVat: 5, net: 875 });
  });

  it('returns all-zero totals for an empty or fully-cancelled list', () => {
    expect(summarizeLiveOwnerSettlements([])).toEqual({ gross: 0, fees: 0, expenses: 0, feeVat: 0, net: 0 });
    expect(summarizeLiveOwnerSettlements([makeSettlement({ status: 'cancelled' })])).toEqual({ gross: 0, fees: 0, expenses: 0, feeVat: 0, net: 0 });
  });
});
