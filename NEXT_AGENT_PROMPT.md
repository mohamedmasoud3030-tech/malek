...
| Branch tip | the docs-only commit that added `HANDOFF.md` + this file (child of the above) |
...
- **G5 — Remaining financial-chain reviews** (not known defects): governed historical adoption/allocation; post-payment adjustment and recovery/offset interfaces showing effect on the original source; remaining S08/S09 sources, cache/rebuild, permissions, read limits, retries, reconciliations; document-surface re-verification after the latest migrations.
...
4. **Step 3 — Close the G5 reviews** across the full chain: expense → source evidence → classification → allocation → offset → settlement → ledger → balance → historical cutoff → reports. Verify against the **deployed** function body, not documentation. Any gap → forward migration + focused SQL test.
