-- ============================================================================
-- ROLLBACK — P1 owner settlement server-side derivation
-- ============================================================================
-- Forward migration: supabase/migrations/20260725000000_p1_owner_settlement_server_derivation.sql
--
-- Scope: drops ONLY the objects introduced by P1 (the derivation RPC, the
-- amount-immutability trigger + its function) and restores
-- create_owner_settlement_draft_atomic to its post-P0 (F-SET) body
-- byte-for-byte. No data is deleted; settlement rows written while P1 was
-- active remain valid ledger records (they were server-derived).
--
-- WARNING (documented by definition of rollback): the restored body re-reads
-- client-sent amounts — re-applying P1 is required to re-close that posture.
-- ============================================================================
...
drop function if exists public.enforce_owner_settlement_amount_immutability();

-- Restores the post-P0 body from
...
-- ACL parity: no grant statements — CREATE OR REPLACE preserves the hardened
-- authenticated-only ACLs from 20260723000000, so the schema surface after this
-- rollback is indistinguishable from pre-P1 main (fingerprint gate).
