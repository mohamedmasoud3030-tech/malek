-- Rollback for: 20260728090000_phase3a1b_canonical_accounts_invoice_payment_receipt_void.sql
-- Phase 3A-1B — restore the pre-migration definitions of the invoice / payment /
-- receipt / VOID family (full signatures, byte-identical bodies captured from the
-- origin/main replayed catalog via pg_get_functiondef).
--
-- Contract:
--   * restores previous definitions per signature (CREATE OR REPLACE keeps the
--     existing grants and pinned search_path);
--   * overloads are preserved exactly: the legacy
--     void_receipt_atomic(uuid, timestamptz, jsonb, jsonb) is untouched (it was
--     untouched by the forward migration as well);
--   * NEVER deletes invoices/payments/receipts/allocations/journal rows, never
...
-- ── restore record_invoice_payment_atomic(payload jsonb) ──────────────────────────────────
...
-- ── restore post_receipt_atomic(payload jsonb) ────────────────────────────────────────────
...
-- ── restore void_receipt_atomic(payload jsonb) ────────────────────────────────────────────
