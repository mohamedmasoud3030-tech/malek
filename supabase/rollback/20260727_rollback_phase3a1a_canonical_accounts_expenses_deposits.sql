-- Restore pre-Phase3A-1A definitions before dropping helpers. No financial rows are deleted.
begin;
...
revoke all on function public.require_company_account_id(uuid,text) from public,anon,authenticated,service_role;
revoke all on function public.ensure_company_account(uuid,text,text) from public,anon,authenticated,service_role;
drop function public.require_company_account_id(uuid,text);
drop function public.ensure_company_account(uuid,text,text);
commit;
