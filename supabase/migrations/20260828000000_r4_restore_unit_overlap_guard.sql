-- ============================================================================
-- R4 fix-forward — restore the unit-overlap guard dropped by 20260823000000
-- ============================================================================
--
-- Caught by the release-blocker pgTAP gate (unit_contract_write_checks):
-- the R4 redefinition of create_contract_atomic preserved the blocked-unit
-- check but DROPPED the inclusive date-overlap rejection that GAP-004
-- carried («الوحدة محجوزة خلال هذه الفترة»). Two draft/active contracts on
-- the same unit could overlap through the create path (the DB exclusion
-- constraint still protects ACTIVE rows, but the business rule also blocks
-- overlapping DRAFTs).
--
-- This migration redefines create_contract_atomic ONLY to reinsert the
-- overlap guard verbatim from GAP-004. Everything else (R4 explicit billing
-- policy, validation order, privileges) is unchanged.
-- ============================================================================
...
comment on function public.create_contract_atomic(text, uuid, uuid, uuid, date, date, numeric, text, uuid, text, text, text, text, integer, integer) is
  'R4 billing-policy contract creation with the GAP-004 unit-overlap guard restored '
  '(«الوحدة محجوزة خلال هذه الفترة»). DRAFT-only creation; activation via the S04 snapshot RPC.';
