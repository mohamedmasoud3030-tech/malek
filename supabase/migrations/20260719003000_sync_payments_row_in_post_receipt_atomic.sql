-- Fix: post_receipt_atomic never inserted a row into public.payments.
-- Consequences:
...
--      any receipt posted through post_receipt_atomic, since no payments row
--      ever existed for it. Voiding was effectively broken for all normal
--      receipts.
--   2. rpt_owner_statement reads exclusively from public.payments, so owner
--      statements never reflected tenant collections recorded via the normal
--      receipt flow (total_gross/total_net stayed 0, transactions stayed []).
--   3. trigger_update_owner_balance_on_payment (which recalculates owner
--      balances) only fires on public.payments writes, so owner balances were
--      never updated on receipt posting.
--
-- Fix: insert one public.payments row per posted receipt, linked via
-- receipt_id, using the receipt's contract_id/amount/channel/date and the
-- first allocation's invoice_id (a receipt's allocations may span invoices;
-- payments.invoice_id is a single-invoice reference used for reporting
-- context only, journal/allocation truth remains in receipt_allocations).
...
BEGIN
  ...
  -- and the owner-balance trigger all have the record they depend on.
