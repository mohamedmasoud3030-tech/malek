-- FOM-013 follow-up — repair reconciliation audit write against the canonical audit_log schema.
-- Forward-only. The prior RPC attempted to insert audit_log.company_id, but that
-- column does not exist in the canonical table, so every otherwise-successful
-- reconciliation match rolled back at the audit step.
...
comment on function public.process_bank_reconciliation_match_atomic(jsonb) is
  'FOM-013: validates reconciliation entity/company/direction/amount/status, prevents duplicate economic matches, requires governed manual-adjustment authority, and writes audit rows using the canonical audit_log schema.';
