...
--   Every identifier comparison is normalized through ::text on both sides so
--   the same guarded cleanup works for the clean UUID baseline and the historic
--   text-backed production layout without broadening the QA graph.
--
-- Safety:
--   - No TRUNCATE.
...
DECLARE
  ...
  v_qa_agreement_id text := '00000000-0000-4000-9002-000000000001';
  ...
  v_bad_count      integer;
BEGIN
  SELECT contract_record.id::text
    INTO v_qa_contract_id
  FROM public.contracts AS contract_record
  WHERE contract_record.property_id::text = v_qa_property_id
    AND contract_record.unit_id::text = v_qa_unit_id
    AND contract_record.tenant_id::text = v_qa_tenant_id
    AND contract_record.agreement_id::text = v_qa_agreement_id
    AND coalesce(contract_record.notes, '') LIKE '%اختبار جاهزية%'
  LIMIT 1;
  ...
  IF v_qa_contract_id IS NULL AND EXISTS (
    ...
    WHERE property_id::text = v_qa_property_id
       ...
       OR agreement_id::text = v_qa_agreement_id
  ) THEN
  ...
  WHERE id::text = v_qa_agreement_id
  ...
  DELETE FROM public.tenant_balances WHERE tenant_id::text = v_qa_tenant_id;
  DELETE FROM public.owner_balances WHERE owner_id::text = v_qa_owner_id;
  ...
  DELETE FROM public.contracts
   WHERE id::text = v_qa_contract_id
     ...
     AND agreement_id::text = v_qa_agreement_id
     AND coalesce(notes, '') LIKE '%اختبار جاهزية%';
