...
--   1. Verify both the DEBIT and CREDIT rows exist with the exact known values.
...
BEGIN
  ...
  -- 3. Safety Check: Verify that the overall matching group contains EXACTLY 2 rows to avoid broader sweep deletion
  ...
  IF v_count <> 2 THEN
    RAISE EXCEPTION 'QA cleanup validation failed: Expected exactly 2 rows matching target criteria, found %', v_count;
  END IF;

  -- 4. Purge the validated rows
