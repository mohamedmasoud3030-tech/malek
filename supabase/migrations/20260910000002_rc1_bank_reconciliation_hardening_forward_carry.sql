-- OPS-002 — carry the RC1 bank/reconciliation hardening (20260901000033) to a
-- database where that migration cannot replay.
--
-- WHY THIS EXISTS
-- 20260901000033 ends with a DO block that rewrites
-- public.wp05_rpt_cash_flow_gl(date,date) by string-matching an exact `v_old`
-- snippet, and aborts with RC1_WP05_CASH_FLOW_GUARD_ANCHOR_NOT_FOUND when that
-- snippet is absent. On the hosted database the anchor is absent for a benign
-- reason: 20260901000064 MOVED that function into app_private and renamed it to
-- financial_cash_flow_gl_core, leaving public.wp05_rpt_cash_flow_gl as a thin
-- permission wrapper. Verified on the hosted database:
--   * app_private.financial_cash_flow_gl_core already contains the exact
--     hardened text this DO block produces -- the `current_user` branch is gone
--     and the guard reads
--       if public.current_company_id() is null or public.current_company_id() <> v_company_id
--     which is byte-identical to 20260901000033's own `v_new`.
--   * so the DO block's objective is ALREADY SATISFIED there; only its anchor
--     lookup is stale. Re-running it would be a no-op at best.
-- Migrations 00064/00066/00067 are applied on that database, i.e. it is AHEAD
-- on this particular function, not behind.
--
-- WHAT WAS ACTUALLY MISSING (proven by statement-level diff, not assumed)
-- Normalizing both definitions of process_bank_reconciliation_match_atomic to
-- whitespace-free statement lists gives 156 statements in the repository versus
-- 132 hosted, with 14 present only in the repository. Those 14 are the RC1
-- hardening itself:
--   * eleven `Cross-company <entity> match rejected.` guards (42501) covering
--     payment, receipt, expense, owner expense, owner payout, deposit receipt,
--     deposit refund, receipt void, deposit refund reversal and commission
--     payment;
--   * duplicate-match guards for receipt and expense (`... already matched.`,
--     23514);
--   * the enriched audit_log write carrying old_value/new_value.
-- The hosted function had ZERO occurrences of "Cross-company".
--
-- RESIDUAL RISK BEFORE THIS MIGRATION: low but real. Every hosted entity lookup
-- is already scoped `... and <alias>.company_id = v_company_id`, so a
-- foreign-company id simply finds no row and fails on the not-found path. The
-- missing guards are defence in depth -- they turn a silent miss into an
-- explicit 42501 and they protect the paths where the amount/company were read
-- before the comparison. Restoring them removes the divergence rather than
-- arguing about its exploitability.
--
-- WHAT THIS MIGRATION IS
-- The verbatim content of 20260901000033 up to and including the accrual grants
-- (its lines 1-776), i.e. everything EXCEPT the obsolete wp05 DO block and the
-- three wp05 grant statements that follow it. Nothing was retyped: the body is
-- copied byte-for-byte, so the two files cannot drift.
--
-- Safe to run on a database where 20260901000033 already applied: every
-- statement in that range is CREATE OR REPLACE, `drop ... if exists` immediately
-- followed by recreation, or an idempotent guard. Forward-only. No data rows are
