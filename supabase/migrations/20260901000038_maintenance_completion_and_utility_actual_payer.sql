...
-- verified financial/operational closure.  Also records who actually paid a
-- utility bill without implying that the office collected that money.
-- Rules: OPS-005, DOM-010, UX-006, UX-008.
...
comment on column public.utility_bills.actual_payer is
  'The party that actually paid the provider. This may differ from charged_to and does not imply office collection.';
...
begin
  ...
  if not found then raise exception 'طلب الصيانة غير موجود' using errcode = 'P0002'; end if;
  ...
  values (gen_random_uuid()::text, extract(epoch from now())::bigint, auth.uid()::text,
    ...
    'maintenance_records', jsonb_build_object('from', v_current, 'to', v_next, 'reason', nullif(btrim(coalesce(p_reason, '')), ''))::text, now());
...
begin
  ...
  v_company_id := public.current_company_id();
  if v_company_id is null then raise exception 'Company context is required.' using errcode = '42501'; end if;
  if not coalesce(p_confirmed, false) then raise exception 'MAINTENANCE_CONFIRMATION_REQUIRED: يجب تأكيد تنفيذ العمل قبل الإغلاق' using errcode = '23514'; end if;
  if p_cost is null or p_cost < 0 or p_cost <> round(p_cost, 3) then raise exception 'MAINTENANCE_COST_INVALID: التكلفة الفعلية يجب أن تكون قيمة موجبة أو صفرية بدقة 3 منازل' using errcode = '22023'; end if;
  if upper(coalesce(p_charged_to, '')) not in ('OWNER', 'TENANT', 'OFFICE') then raise exception 'MAINTENANCE_CHARGE_TARGET_INVALID: حدد المالك أو المستأجر أو المكتب' using errcode = '22023'; end if;

  select * into v_row from public.maintenance_records
   where id::text = p_request_id and company_id = v_company_id and deleted_at is null for update;
  if not found then raise exception 'طلب الصيانة غير موجود' using errcode = 'P0002'; end if;
  if lower(coalesce(v_row.status, '')) <> 'resolved' then raise exception 'MAINTENANCE_CLOSE_REQUIRES_COMPLETION: لا يمكن الإغلاق قبل تسجيل تم التنفيذ' using errcode = '23514'; end if;

  perform set_config('malek.maintenance_transition_sanctioned', 'true', true);
  update public.maintenance_records set
    status = 'closed', cost = p_cost, charged_to = upper(p_charged_to),
  ...
  insert into public.audit_log (id, ts, user_id, action, entity, entity_id, note, "table", details, created_at)
  values (gen_random_uuid()::text, extract(epoch from now())::bigint, auth.uid()::text,
    'CLOSED_VERIFIED', 'maintenance_records', p_request_id, 'إغلاق صيانة بعد تحقق تشغيلي ومالي',
    'maintenance_records', jsonb_build_object('cost', p_cost, 'charged_to', upper(p_charged_to), 'evidence_url', nullif(btrim(p_evidence_url), ''), 'confirmed', true)::text, now());
  return jsonb_build_object('maintenance', (select to_jsonb(m) from public.maintenance_records m where m.id = v_row.id), 'expense_id', v_row.expense_id);
end;
$$;

revoke all on function public.close_maintenance_with_expense(text, numeric, text, text, text, boolean) from public;
grant execute on function public.close_maintenance_with_expense(text, numeric, text, text, text, boolean) to authenticated;
comment on function public.transition_maintenance_status_atomic(text, text, text) is 'P3 lifecycle: report → in progress → work completed. Final closure is handled by close_maintenance_with_expense after verification.';
comment on function public.close_maintenance_with_expense(text, numeric, text, text, text, boolean) is 'P3 final maintenance closure: requires completed work, actual cost, charge target and human confirmation; records optional evidence.';
