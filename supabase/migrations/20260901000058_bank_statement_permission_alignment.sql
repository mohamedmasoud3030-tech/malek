...
-- financial.bank_reconciliation.match is part of the role matrix and owner
-- overrides/grants are authoritative. Keep all validation/company/idempotency
-- semantics from migration 57 and replace only the authorization decision.
...
begin
  ...
  if not coalesce(public.current_user_has_effective_app_permission('financial.bank_reconciliation.match'), false) then
    raise exception 'Bank statement line creation requires financial.bank_reconciliation.match permission (ADMIN or MANAGER are not the only eligible roles).' using errcode = '42501';
  end if;
...
comment on function public.create_bank_statement_line_governed(jsonb) is
  'Governed manual bank statement line creation. Requires effective financial.bank_reconciliation.match permission, company-scoped, duplicate-fingerprint protected, always created unmatched.';
...
begin
  ...
  if not coalesce(public.current_user_has_effective_app_permission('financial.bank_reconciliation.match'), false) then
    raise exception 'Ignoring a bank statement line requires financial.bank_reconciliation.match permission (ADMIN or MANAGER are not the only eligible roles).' using errcode = '42501';
  end if;
...
comment on function public.ignore_bank_statement_line_governed(uuid) is
  'Governed bank statement line ignore. Requires effective financial.bank_reconciliation.match permission, company-scoped, only from unmatched status.';
