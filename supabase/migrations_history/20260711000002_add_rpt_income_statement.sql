-- Migration: add_rpt_income_statement
-- Description: Production-ready accounting foundation report.
--
-- Presents an accrual-basis Income Statement for a date range. Revenue is the
-- invoiced (accrued) rental/operational revenue in the period; expenses are
-- operational expenses incurred in the period, broken down by category. VAT is
-- intentionally excluded here (reported separately via rpt_vat_return) so the
-- P&L reflects operating performance only.
--
