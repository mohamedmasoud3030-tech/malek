-- Two bugs:
-- 1) contracts.id is text but p_contract_id is uuid - "where c.id=p_contract_id"
--    had no operator and raised a hard error on every call.
-- 2) It joined a "tenants" table (40 live rows, untracked, no FK relationship
--    to contracts) instead of "people" - contracts.tenant_id has a real FK
--    to people(id) (contracts_tenant_id_people_app_fkey), which is also what
--    create_contract_atomic validates against. "tenants" is legacy/orphaned
--    for this purpose. Switched the join to people; full_name replaces name.
-- Not wired into the frontend yet, so zero live impact. Signature unchanged.
