-- ============================================================================
-- P1 follow-up — property text identifier compatibility for owner workflows
-- ============================================================================
-- Scope:
--   * properties.id is text in the live Rentrix schema; never force it through
--     uuid casts in owner-agreement or owner-settlement write paths.
--   * keep owner_id as the existing UUID-facing contract, but compare all
--     property identifiers via ::text and store them through target-column types.
--   * replace the optional property filter overload of calculate_owner_net_payout
--     with a text property argument and remove the stale uuid overload to avoid
--     PostgREST/RPC overload ambiguity.
--
-- No table, column, policy, production row, or historical ledger mutation.
-- ============================================================================
...
begin
  ...
  if p_property_id is not null and not exists (
    ...
    where p.id::text = p_property_id and p.company_id = v_company_id and p.deleted_at is null
  ) then
...
revoke all on function public.calculate_owner_net_payout(uuid, date, date, text) from public, anon;
grant execute on function public.calculate_owner_net_payout(uuid, date, date, text) to authenticated, service_role;

comment on function public.calculate_owner_net_payout(uuid, date, date, text) is
...
declare
  v_company_id uuid;
  v_owner_id public.owner_agreements.owner_id%type;
  v_property_id public.owner_agreements.property_id%type;
  v_row public.owner_agreements%rowtype;
begin
  if auth.uid() is null or not public.is_admin_or_manager() then
    raise exception 'غير مصرح: يحتاج هذا الإجراء صلاحية مدير أو مشرف.' using errcode = '42501';
  end if;
  ...
  v_owner_id := nullif(payload->>'owner_id', '');
  v_property_id := nullif(payload->>'property_id', '');

  if v_owner_id is null or v_property_id is null then
    raise exception 'owner_id and property_id are required.' using errcode = '22023';
  end if;

  if not exists (
    select 1
    from public.owners o
    where o.id::text = v_owner_id::text
      and o.company_id = v_company_id
      and o.deleted_at is null
  ) then
  ...
  if not exists (
    ...
    where pr.id::text = v_property_id::text
  ...
  insert into public.owner_agreements (
    owner_id, property_id, agreement_type, commission_type, commission_value,
    starts_on, ends_on, notes, company_id
  ) values (
    v_owner_id,
    v_property_id,
...
-- Remove the unsafe optional-property overload after the replacement functions
-- compile. Leaving both overloads would make PostgREST RPC resolution ambiguous.
drop function if exists public.calculate_owner_net_payout(uuid, date, date, uuid);
