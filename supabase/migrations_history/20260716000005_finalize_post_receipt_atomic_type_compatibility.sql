-- Finalize post_receipt_atomic compatibility across clean UUID and historical
-- text identifier layouts. The public JSON facade and financial behavior remain
-- unchanged; values are coerced through target-column %TYPE variables before
-- inserts, while lookups compare identifier text representations explicitly.
...
GRANT EXECUTE ON FUNCTION public.post_receipt_atomic(jsonb)
