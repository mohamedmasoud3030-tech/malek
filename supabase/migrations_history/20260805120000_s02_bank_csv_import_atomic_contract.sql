-- ============================================================================
-- S02-T07/T08: bank CSV import — restore the deterministic error contract and
-- make batch+line writes truly atomic.
--
-- Two root defects in 20260805000001_bank_csv_import_hardening.sql:
--
-- 1. The zero-amount guard raised `Amount must be non-zero at row %` INSIDE a
--    nested `begin ... exception when others` block, so its own message was
--    swallowed and re-raised as `Invalid amount at row %: 0`. The canonical
--    contract (22023 + deterministic message) is now surfaced by performing
--    the zero check outside the numeric cast handler.
--
-- 2. Line inserts were wrapped in `exception when others then continue`, so a
--    real line failure was silently swallowed while the batch row already
--    claimed server counts — the write was not atomic. The swallow is
--    removed: any line error now aborts the whole function transaction, so an
--    invalid batch writes nothing. Exact duplicate fingerprint races remain
--    handled by `on conflict ... do nothing`.
--
-- Also widens bank_statement_lines.amount and .balance to numeric(14,3) so
-- OMR 3-decimal (baisa) precision is preserved end to end; the row
-- fingerprint already canonicalised to 3 decimals and must not be silently
-- rounded to 2 on storage.
--
-- Company isolation, idempotent retry by (company_id, file_fingerprint),
-- server-recomputed counts, in-file duplicate counting, possible-duplicate
-- detection, grants and search_path pinning are preserved exactly.
-- No GL posting tables are touched (no journal_entries / journal_batches).
-- Rollback: supabase/rollback/20260805_rollback_s02_bank_csv_import_atomic_contract.sql
-- ============================================================================

begin;

-- 1. OMR 3-decimal precision on stored line amounts --------------------------
alter table public.bank_statement_lines
  alter column amount type numeric(14,3) using round(amount::numeric, 3);

alter table public.bank_statement_lines
  alter column balance type numeric(14,3) using round(balance::numeric, 3);

-- 2. Atomic import RPC with the deterministic error contract -----------------
...
comment on function public.import_bank_statement_batch_atomic(jsonb) is 'S02: atomic staged bank CSV import with idempotency, duplicate detection, OMR 3dp precision, deterministic 22023 error contract and company isolation. Batch and lines commit together; an invalid batch writes nothing. No accounting postings.';
