...
begin
  ...
  insert into public.contracts (
    ...
    p_property_id, p_unit_id, p_tenant_id::text, p_agreement_id,
    p_start_date::text, p_end_date::text, p_rent_amount,
    p_payment_cycle, p_payment_terms_id::text, p_status,
    p_cancellation_reason, p_notes, p_attachment_url
  )
  returning id into v_contract_id;

  return (select to_jsonb(c) from public.contracts c where c.id = v_contract_id);
end;
$function$;
