-- Migration: fix_record_invoice_payment_void_receipt_shared_id
--
-- Decision: Option A. record_invoice_payment_atomic now generates one UUID and
-- uses it as both payments.id and receipts.id for the same invoice-payment
-- operation. The Receipts UI is payment-backed and void_receipt_atomic resolves
-- receipts.id, so separate UUIDs broke every production void attempt.
--
-- The functions remain idempotent through financial_operation_idempotency and
-- receipt request_id checks. Grants are preserved without broadening anonymous
-- access beyond the existing jsonb facade grant.
