...
select is((select numeric_scale from information_schema.columns where table_schema='public' and table_name='master_lease_measurements' and column_name='initial_liability'),3,'liability is OMR 3dp');
select is((select numeric_scale from information_schema.columns where table_schema='public' and table_name='master_lease_schedule_rows' and column_name='interest_amount'),3,'interest is OMR 3dp');
select is((select numeric_scale from information_schema.columns where table_schema='public' and table_name='master_lease_schedule_rows' and column_name='rou_depreciation'),3,'depreciation is OMR 3dp');
...
select is((public.gl_ml_measure_payments('[{"period":1,"amount":100.000},{"period":2,"amount":100.000}]'::jsonb,0,12)->>'max_period')::integer,2,'max period server-derived');
...
select ok(pg_get_functiondef('public.gl_ml_post_sublease_receipt(jsonb)'::regprocedure) like '%agreement_type=''master_lease''%' and pg_get_functiondef('public.gl_ml_post_sublease_receipt(jsonb)'::regprocedure) like '%''4000''%' and pg_get_functiondef('public.gl_ml_post_sublease_receipt(jsonb)'::regprocedure) not like '%''2000''%','sublease is principal revenue, never owner payable');
