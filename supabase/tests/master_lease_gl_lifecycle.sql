...
-- Full behavioral lifecycle (39..64) -----------------------------------------
...
values('06000000-0000-4000-8000-000000000001','S06 Owner','S06 Owner','c6000000-0000-4000-8000-000000000001');

insert into public.properties(id,title,name,type,address,company_id)
values('16000000-0000-4000-8000-000000000001','S06 Property','S06 Property','residential','Muscat','c6000000-0000-4000-8000-000000000001');
...
insert into public.owner_agreements(
  ...
  '26000000-0000-4000-8000-000000000001','06000000-0000-4000-8000-000000000001',
  '16000000-0000-4000-8000-000000000001','master_lease','FIXED_MONTHLY',0,date '2026-08-01',
  'c6000000-0000-4000-8000-000000000001'
);

insert into public.units(id,property_id,name,unit_number,company_id)
values('36000000-0000-4000-8000-000000000001','16000000-0000-4000-8000-000000000001','S06 Unit','ML-1','c6000000-0000-4000-8000-000000000001');
...
insert into public.contracts(
  ...
  '56000000-0000-4000-8000-000000000001','16000000-0000-4000-8000-000000000001',
  '36000000-0000-4000-8000-000000000001','46000000-0000-4000-8000-000000000001',
  '26000000-0000-4000-8000-000000000001',date '2026-08-01',date '2027-07-31',80,'active',
  'c6000000-0000-4000-8000-000000000001'
);
...
select throws_ok(
  $test$select public.gl_ml_create_initial_measurement('{"company_id":"c6000000-0000-4000-8000-000000000002","owner_agreement_id":"26000000-0000-4000-8000-000000000001","request_id":"s06-cross-company","effective_date":"2026-08-01","annual_discount_rate_bps":0,"periods_per_year":12,"payments":[{"period":1,"amount":100},{"period":2,"amount":100}]}'::jsonb)$test$,
  '42501','GL_ML_AGREEMENT_NOT_FOUND_OR_NOT_MASTER_LEASE',
  'cross-company master-lease agreement use is rejected'
);

select lives_ok(
  $test$select public.gl_ml_create_initial_measurement('{"company_id":"c6000000-0000-4000-8000-000000000001","owner_agreement_id":"26000000-0000-4000-8000-000000000001","request_id":"s06-initial","effective_date":"2026-08-01","annual_discount_rate_bps":0,"periods_per_year":12,"payments":[{"period":1,"amount":100},{"period":2,"amount":100}]}'::jsonb)$test$,
  'initial measurement is created'
);
select lives_ok(
  $test$select public.gl_ml_create_initial_measurement('{"company_id":"c6000000-0000-4000-8000-000000000001","owner_agreement_id":"26000000-0000-4000-8000-000000000001","request_id":"s06-initial","effective_date":"2026-08-01","annual_discount_rate_bps":0,"periods_per_year":12,"payments":[{"period":1,"amount":100},{"period":2,"amount":100}]}'::jsonb)$test$,
  'identical initial request is idempotent'
);
...
select lives_ok(
  $test$select public.gl_ml_post_initial_recognition(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-initial'),'cash_account_no','1120'))$test$,
  'initial recognition posts through canonical GL'
);
...
select lives_ok(
  $test$select public.gl_ml_post_period(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-initial'),'period_no',1,'cash_account_no','1120'))$test$,
  'first lease period posts'
);
...
select lives_ok(
  $test$select public.gl_ml_create_remeasurement('{"company_id":"c6000000-0000-4000-8000-000000000001","owner_agreement_id":"26000000-0000-4000-8000-000000000001","request_id":"s06-remeasure","effective_date":"2026-09-01","annual_discount_rate_bps":0,"periods_per_year":12,"scope_reduction_bps":0,"payments":[{"period":1,"amount":60},{"period":2,"amount":60}]}'::jsonb)$test$,
  'remeasurement freezes posted carrying values'
);
...
select throws_ok(
  $test$select public.gl_ml_post_period(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-initial'),'period_no',2,'cash_account_no','1120'))$test$,
  '22023','GL_ML_PERIOD_BLOCKED_BY_PENDING_REMEASUREMENT',
  'pending remeasurement blocks future old-schedule posting'
);
select lives_ok(
  $test$select public.gl_ml_post_remeasurement(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-remeasure')))$test$,
  'remeasurement posts through canonical GL'
);
...
select lives_ok(
  $test$select public.gl_ml_post_period(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-remeasure'),'period_no',1,'cash_account_no','1120'))$test$,
  'first revised period posts'
);
...
select lives_ok(
  $test$select public.gl_ml_post_sublease_receipt('{"company_id":"c6000000-0000-4000-8000-000000000001","contract_id":"56000000-0000-4000-8000-000000000001","source_id":"s06-sublease","amount":80,"effective_date":"2026-10-01","cash_account_no":"1120"}'::jsonb)$test$,
  'sublease receipt posts as principal revenue'
);
...
select lives_ok(
  $test$select public.gl_ml_create_remeasurement('{"company_id":"c6000000-0000-4000-8000-000000000001","owner_agreement_id":"26000000-0000-4000-8000-000000000001","request_id":"s06-terminate","effective_date":"2026-10-01","annual_discount_rate_bps":0,"periods_per_year":12,"scope_reduction_bps":10000,"payments":[]}'::jsonb)$test$,
  'full termination draft is created at posted boundary'
);
select lives_ok(
  $test$select public.gl_ml_post_remeasurement(jsonb_build_object('company_id','c6000000-0000-4000-8000-000000000001','measurement_id',(select id from public.master_lease_measurements where request_id='s06-terminate')))$test$,
  'full termination posts'
);
