...
| PX-05 REGULATORY SPEC | Manager/Operations | كان لا يوجد سجل structured للتسجيل | التسجيل الرسمي وإثباته منفصلان عن `ACTIVE` | نزاع/عدم امتثال؛ BLOCKER قبل ادعاء Oman-ready | **نُفذ الإطار fail-closed** | profile effective-dated + submit + distinct verification + evidence + events | لا hard-coded fee/deadline؛ UI يعرض NOT_CONFIGURED حتى legal profile معتمد؛ hosted/legal proof متبقٍ |
| PX-06 MISSING EXPECTED CAPABILITY | Operations/Tenant/Owner | كان المستند بلا handover workflow structured | فحص، عدادات، مفاتيح، صور وتوقيعات قبل/بعد | نزاع تأمين وصيانة؛ HIGH | **نُفذت الرحلة في repository** | system templates + drafts + conditions/meters/keys/docs/signatures + distinct review | damage claim يتطلب MOVE_OUT مراجع؛ cross-company/self-review blocked؛ current-SHA browser proof متبقٍ |
...
المنتج الصحيح هو **office operating system موثوق مالياً**. لا يحتاج ميزات أكثر قبل الإطلاق؛ يحتاج إغلاق الأدلة الخارجية والإطاران الناقصان للتسجيل والفحص أصبحا منفذين في repository ومغلقين افتراضياً قانونياً. المتبقي لإطلاقهما: legal profile معتمد، hosted browser proof، ثم live pilot. Password recovery يحتاج hosted email/redirect proof. أي توسع CRM/marketplace/portal الآن يقلل فرصة إطلاق آمن.
