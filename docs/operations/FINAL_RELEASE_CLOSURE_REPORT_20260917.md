...
>
> This report describes the state of the **4-blocker phase** and pins the
> release candidate at `828ac232`. That candidate was never deployed, and the
> findings above have since been carried forward and re-measured:
>
> * the source of the owner-report contradiction was found and fixed at the
>   database and service layer (`1a22e7af`), not by relabelling output;
> * the workspace was re-based onto the current `main` (`8c2605e0`), so this
>   report's "branch" and "deployment" lines no longer describe the tip;
> * Blocker 1 (GitHub Actions billing) and the Supabase parity blocker were
>   re-measured and are recorded in
>   `PHASE4_CONVERGENCE_REPORT_20260917.md`, which supersedes this file.
>
> The measured facts below remain accurate **for `828ac232` only**. Do not
