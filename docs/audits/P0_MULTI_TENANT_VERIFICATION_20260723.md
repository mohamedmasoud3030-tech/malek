...
| `build` | ✅ |
| `check:architecture` | ✅ |
| حزم P0 المستقلة (`src/p0/`) | ✅ 11/11 ملف — 56 اختبارًا (سلوكيّ قبل/بعد + عزل السبب + العقد + forward/rollback) |
...
الحالة الآن: **GO مشروط** — P0 مكتمل تقنيًا: كل ثغرة عزل مؤكدة مغلقة باختبارات سلوكية قبل/بعد، Rollback 19/19 وبصمة مطابقة، كل البوابات المحلية خضراء. الشروط المتبقية حوكمية لا تقنية: اكتمال خُضر CI على PR #1276 ثم الدمج (squash) إلى main. **لا يبدأ P1** (خلل أرقام التسوية الموثوقة من العميل — إثبات مستقل قائم في `behavioral-isolation.json`: قبول net_payable=999999 قبل الإصلاح وبعده عمدًا) **إلا بعد الدمج ومن أحدث main في PR مستقل.**
