...
الحالة الآن: **GO** — P0 مكتمل: كل ثغرة عزل مؤكدة مغلقة باختبارات سلوكية قبل/بعد، Rollback 19/19 وبصمة مطابقة، والانحدار التشغيلي الوحيد الذي أمسكته بوابة الإصدار (42803 في ميزان العقود) أُصلح وتحقق محليًا بفرق أعطال صِفر. كل البوابات خضراء: محليًا (1000 اختبار كامل + 57 P0) وعلى CI لكل SHA (بما فيه `isolated-replay` و`release-blocker-database` على `02696f6`؛ أعادهما أخضر بعد الإصلاح). المتبقي خطوة حوكمية واحدة: الدمج (squash) إلى main ثم فحص إنتاجي آمن قراءة‑فقط للتأكد الحي. **لا يبدأ P1** (خلل أرقام التسوية الموثوقة من العميل — إثبات مستقل قائم في `behavioral-isolation.json`: قبول net_payable=999999 قبل الإصلاح وبعده عمدًا) **إلا بعد الدمج ومن أحدث main في PR مستقل.**
