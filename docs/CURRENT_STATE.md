...
**Resolved follow-up (live re-verified 2026-07-12):** migration `20260712020000_fix_tenant_balances_people_fk` retargeted `tenant_balances.tenant_id` from legacy `tenants(id)` to canonical `people(id)`. Direct read-only inspection confirms the validated `tenant_balances_tenant_id_people_fkey` constraint with `ON DELETE RESTRICT`, and the legacy FK is absent. The check also found 10 `people` rows with no matching legacy `tenants` row; the corrected constraint supports those current official-flow tenants.
