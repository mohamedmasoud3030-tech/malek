...
**Orphaned live-schema enum types (deferred, not created by any migration):** live introspection of `pg_type`/`pg_enum` found 13 enum types total on `nnggcnpcuomwfuupupwg`, but only 4 are referenced by any live column (the ones captured in `20260705000000_...`). The other 9 — `contract_status`, `invoice_status`, `invoice_type`, `journal_entry_type`, `maintenance_status`, `payment_method`, `property_status`, `transaction_status`, `unit_status` — exist live but are not used by any table's column today. They are intentionally **not** created by any baseline migration and **not** dropped from production. Before deciding whether to formally capture or drop them, someone should audit whether they're referenced outside table columns (function signatures, view definitions, RLS policy expressions cast to the type) — a column-only check would miss those. Tracked as a follow-up cleanup PR, out of scope for the untracked-tables baseline work.
