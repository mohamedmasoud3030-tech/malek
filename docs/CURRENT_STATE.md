...
### Owner/property/agreement contract repair (2026-07-18)

- Production previously had 10 active properties and 10 ownership links but zero `owner_agreements`, so the contract RPC's required agreement invariant could not be satisfied for existing properties. Live migrations `20260718113322`, `20260718113340`, `20260718113405`, and `20260718113414` are now applied.
- All 10 managed properties now have an agreement with full temporal ownership coverage: 8 preserve `FIXED_MONTHLY` and 2 preserve `RATE`; there are zero uncovered properties, agreement/ownership mismatches, owner name mismatches, or property name mismatches.
