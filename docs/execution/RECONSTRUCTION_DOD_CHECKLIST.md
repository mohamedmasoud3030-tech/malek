...
|---|---|---|---|
| G1 — authenticated hosted E2E | **PARTIALLY UNBLOCKED 2026-09-11 ~10:05Z** | User supplied a PAT; push succeeded (`bcdf6944..a99e4295`), remote SHA verified via `git ls-remote` == local HEAD. Hosted runs dispatched on the pushed SHA: ci.yml run 34579002427 + browser-readiness.yml run 34579004391 (full self-contained Playwright matrix incl. the NOW-11/12 panel specs; run_staging=false, no secrets required). Outcomes recorded in HANDOFF §K when complete. | Remaining: hosted RUNTIME access (deployed app + Supabase credentials) for authenticated hosted E2E against live data — a GitHub PAT does not provide it |
...
| G4 — runtime of newly applied migrations under real traffic | **NOT PROVEN (hosted-blocked)** | Local replay: 100/100 migrations replay clean (NOW-4 evidence; 0 schema changes since). Real-traffic behaviour cannot be exercised locally | Hosted traffic/preview environment |
