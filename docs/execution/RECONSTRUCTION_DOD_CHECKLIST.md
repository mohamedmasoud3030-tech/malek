...
|---|---|---|---|
| G1 — authenticated hosted E2E | **BLOCKED** | No GitHub/host credential anywhere in sandbox (verified exhaustively: env, credential files, helpers, gh, SSH, /run/secrets). `git push` → `could not read Username` | User supplies a PAT or hosted access; then push + hosted runs |
