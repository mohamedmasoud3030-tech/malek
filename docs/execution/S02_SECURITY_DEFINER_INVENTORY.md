...
> Generated by `scripts/generate-s02-security-definer-inventory.mjs` on 2026-08-07.
> Base: latest `origin/main` at `d717ac6c7bd312d510dc8982ddf37e17a452857c`.
...
| `import_bank_statement_batch_atomic` | 20260807160000_s02_bank_csv_import_server_guards.sql | DEFINER | public, pg_temp | current_company_id() + company_id scoped write | revoked | yes |
...
## S02-T07/T08 verification

Migration `20260807160000_s02_bank_csv_import_server_guards.sql` keeps the
atomic import RPC as the only server write path for bank CSV lines and adds the
missing untrusted-payload guards: required positive `file_size`, 5 MiB maximum
file size, 5,000-row maximum batch size, OMR-only currency, maximum 3 decimal
places for amounts and balances, and fail-closed rejection when a row carries
both debit and credit. Regression evidence lives in
`supabase/tests/bank_csv_import_fail_closed.sql` and
