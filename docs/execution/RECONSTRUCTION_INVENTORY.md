...
- ⚠️ **Migration drift partly closed: 20 of 27 applied, halted at a real drift.** Ledger 79 → 98, **zero data change** (every row count identical to the pre-apply baseline). Halted at `20260909000015` because the hosted `process_bank_reconciliation_match_atomic` is genuinely missing the `20260901000033` fail-closed hardening (12,961 vs 16,662 chars) — not cosmetic. Re-applying `033` hit a *nested* anchor guard (`RC1_WP05_CASH_FLOW_GUARD_ANCHOR_NOT_FOUND`), so work stopped rather than improvising past safety guards. **7 remain:** `20260909000015`–`...021`, so production still lacks OMR 3-decimal precision.
