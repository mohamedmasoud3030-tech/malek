...
- **Money precision, re-corrected 2026-09-13 (third time — the last one was still incomplete).** Re-measured directly against live Production metadata and against a clean 104-migration replay: the `numeric(18,3)` set is **identical on both sides (74 columns: 73 tables + 1 view)**, and no *stored* money value is at risk. But the "none is money" conclusion missed a **write-path** case: `properties.current_value` is `numeric(14,2)` while the property form renders it with the shared OMR money step (`MONEY_STEP = getCurrencyStep('OMR') = '0.001'`) and `propertySchema` validates it with an unconstrained `optionalMoney`, so `760000.123` is accepted by the UI and stored as `760000.12` (proved with a read-only cast on Production; `0.001` → `0.00`). Its sibling `purchase_value` is already `numeric(18,3)`, so the two halves of any capital-gain figure sit at different precisions. Fixed forward by `20260913000001_owner_valuation_omr_precision.sql` (lossless widening, preflight + postcondition, idempotent). **No accounting rule, no stored value and no aggregate changes; only the silent input truncation is removed.** Post-change replay: `numeric(18,3)` = 75, scale-<3 = 6, money-named scale-<3 = **0**. Until Production applies it (owner-gated, see the ledger note), live reports **74 / 7 scale-2**, so "repo == production precision" is temporarily false by exactly this one column.
