...
begin
  ...
  if v_matched_entity_type not in ('payment','receipt','expense','manual_adjustment','owner_payout','deposit_receipt','deposit_refund','commission_payment','owner_expense') then raise exception 'Invalid matched_entity_type.' using errcode = '22023'; end if;
  ...
  if v_matched_entity_type='payment' then
    ...
    if lower(v_entity_status) in ('voided','reversed','void') then raise exception 'Cannot match reversed/voided receipt.' using errcode='23514'; end if;
    ...
    if exists (select 1 from public.bank_reconciliation_matches m where m.matched_entity_type='receipt' and m.matched_entity_id=v_matched_entity_id and m.company_id=v_company_id) then raise exception 'Receipt already matched.' using errcode='23514'; end if;
  ...
  values(v_statement_line_id,v_matched_entity_type,v_matched_entity_id,v_matched_amount,v_notes,auth.uid(),v_company_id)
  ...
  insert into public.audit_log(id,user_id,action,entity,entity_id,note,"table",old_value,new_value,action_timestamp,created_at,updated_at,company_id)
  values(gen_random_uuid()::text,auth.uid(),'PROCESS_BANK_RECONCILIATION_MATCH_ATOMIC','bank_reconciliation_match',v_match.id::text,'Bank statement line matched atomically through RPC with full validation (FOM-013 B6).','bank_reconciliation_matches',to_jsonb(v_line),jsonb_build_object('match',to_jsonb(v_match),'statement_line_status','matched'),now(),now(),now(),v_company_id);
...
comment on function public.process_bank_reconciliation_match_atomic(jsonb) is 'FOM-013 B6: validates entity exists, same company, cash/bank direction, amount, status eligibility (PAID/POSTED/COMPANY/OWNER), rejects reversed/voided, prevents duplicate economic match, requires real authority for manual_adjustment';
