...
begin
  ...
  if upper(coalesce(new.document_status, 'DRAFT')) = 'POSTED'
    ...
    raise exception 'INVOICE_POSTING_REQUIRES_ACTIVE_CONTRACT: invoice % cannot be posted while contract % is %.', new.id, new.contract_id, coalesce(v_contract_status, 'NULL')
      using errcode = '23514';
  end if;
