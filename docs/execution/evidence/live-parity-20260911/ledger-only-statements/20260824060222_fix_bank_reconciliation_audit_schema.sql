...
comment on function public.process_bank_reconciliation_match_atomic(jsonb) is 'FOM-013: validates reconciliation entity/company/direction/amount/status, prevents duplicate economic matches, requires governed manual-adjustment authority, and writes audit rows using the canonical audit_log schema.';
commit;