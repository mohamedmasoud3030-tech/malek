...
| OPS-007 | Maker-Checker for material approvals | contract maker/checker; settlement maker/checker; receipt maker; permission requests | contract/settlement/void/review RPCs with identity separation | settlement and void workflows enforce separation | self-approval denial in backend for contracts, settlements, voids and permission reviews | maker-checker pgTAP; permission workflow tests; settlement/void maker-checker tests | designated full set unverified in live deployment | IMPLEMENTED_UNVERIFIED | GAP-002 | WP-01 |
...
| SEC-004 | Six product roles | `20260811120000` six-role CHECK + helpers; `role_has_app_permission` six-role matrix | `permissions.ts` six roles; `role_has_app_permission` six roles | labels/guards for all six roles | `is_admin()`, `is_admin_or_manager()`, `is_accountant()`, `is_operations()`, `is_viewer()`, `is_read_only_role()` | 20-role permission tests, 3-model tests, 40-pgTAP role matrix tests | no live migration performed | IMPLEMENTED_UNVERIFIED | GAP-001 | WP-01 |
...
| SEC-008 | Maker-Checker backend identity separation | contract/checker; permission-review; settlement maker/checker; receipt maker | contract/review RPCs; `approve_owner_settlement_atomic`; `pay_owner_settlement_atomic`; `receipt_void_maker_checker_guard` trigger | settlement/void workflows enforce maker-checker in RPC | self-approval rejection for contracts, permissions, settlements and receipt void | maker-checker pgTAP; permission tests; 40-role-matrix pgTAP | designated full set unverified in live deployment | IMPLEMENTED_UNVERIFIED | GAP-002 | WP-01 |
...
|---|---|---|---|---|---|---|---|---|---|
| GAP-001 | BLOCKER→PARTIAL | SEC-004 | role assignment cannot express approved job separation | fail-open/fail-closed migration errors may widen or remove access | ADR0015 vs `permissions.ts` three-role catalog → six-role model implemented in `permissions.ts`, migration `20260811120000`, DB helper functions, pgTAP tests | migrate storage/JWT/RLS/UI to six roles with explicit compatibility mapping → repository implementation complete; live migration and hosted verification not performed | owner-approved role mapping; live data inventory; hosted deployment credentials | WP-01 | Yes |
| GAP-002 | BLOCKER→PARTIAL | OPS-007, SEC-008 | designated approvals are inconsistently separated | self-approval of VOID/settlement/material action | contract Maker-Checker exists; broader authoritative inventory absent → contracts, permissions, settlements (approve/pay) and receipt void all enforce maker-checker identity separation at RPC/trigger level | cover every designated action, audited sole-admin override and UI workflow → repository enforcement complete for contracts, settlements, voids, permissions; sole-admin override not implemented; UI workflow not complete; live verification not performed | GAP-001 capability model; action inventory; sole-admin override decision; hosted verification | WP-01 | Yes |
...
| GAP-018 | HIGH→PARTIAL | SEC-009 | sensitive mutation surface remains uncertain | browser bypass of RPC/audit/GL controls | boundary guards exist; legacy deposit and distributed SDK writes remain → `check-sensitive-financial-write-boundary.mjs` regression guard added; pgTAP direct-write hardening test exists; legacy deposit paths remain until GAP-009 closes | exhaustive sensitive-write inventory with zero unintended direct writes → regression guard prevents new browser writes to sensitive financial tables; legacy deposit direct-write path remains open for WP-02 | domain owners; GAP-009/011 | WP-01 | Yes |
