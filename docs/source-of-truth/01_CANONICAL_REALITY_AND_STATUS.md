...
| **F9** | Deferred Revenue Report | Reports | Accrual deferred revenue tracking | Renders report; uses mocked collections data (PL-001). | Yes | Yes | No | `src/features/reports/reports-page.tsx` | Remove `sampleCollections` mock and wire to real collections. | High | Critical gap (PL-001); write real accrual scheduler. | `PARTIALLY_IMPLEMENTED` |
