...
| **E7** | Owner Settlements | Finance | Calculate and disburse owner net payout | Renders settlements page. Derives net; atomic collection reservations. | Yes | Yes | No | `src/features/owners/owner-settlements-page.tsx` | Requires manual first-run ADMIN supervision; material edge-case gaps (e.g. negative balance collection OD-08) remain open. | High | Run pilot with ADMIN eyes on first payout; resolve collection rules. | `PARTIALLY_IMPLEMENTED` |
