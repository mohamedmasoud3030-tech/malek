...
| `owner_agreements` | `owner_agreement_versions` | one identity has ordered versions; at most one unsuperseded current version | atomic identity+first-version RPCs; `create_owner_agreement_version_atomic` | RC1 creation is atomic, the UI exposes current/history and only future amendments, generated types and regression tests cover the chain |
| `owner_agreement_versions` | `contracts` | many contracts may snapshot one applicable agreement version; snapshot is immutable after activation | contract approval/activation RPCs | React service/UI lifecycle and database snapshot authority are wired; hosted/legal acceptance remains external |
| `contracts` | `contract_registration_records` + effective profile | registration evidence is separate from operational status; authority/legal reference/deadline/fee are snapshotted from a legally approved company profile | submit + distinct verify/reject RPCs; profile is empty by default | Repository/UI implemented; no Oman profile is seeded or implied; legal activation remains external |
| `contracts` | `contract_inspections` + templates/events | move-in/out evidence snapshots its template; completed evidence carries both signatures and distinct review; changes return to draft without deleting history | draft/complete/review RPCs; company RLS; event log | system operating checklist exists; reviewed MOVE_OUT evidence is required by the new DAMAGE-claim browser boundary |
