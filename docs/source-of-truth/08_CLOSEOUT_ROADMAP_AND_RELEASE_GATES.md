...
- Recurring OWNER_AGENCY invoices derive immutable contract model/collection-role lineage: OWNER creditor issuance is operational-only; OFFICE creditor is Dr 1201 / Cr 2000 (+2100 from original profile), and neither maps ordinary owner rent to 4000.
- Rent posting blocks a missing effective rent profile; RATE/FIXED fee recognition independently blocks missing versioned fee-tax treatment. Both snapshot configured treatment/code/rate/basis/amount; credit and collection reuse original source lineage rather than a later policy.
- VOID/credit/refund/termination paths preserve history and reconcile subledger/GL/report totals; 2000 uses an S08-approved opening/cutover balance plus append-only post-cutover `owner_funds_events`, and fails closed where historical 2000 sources lack that baseline.
...
| G2 — No release-blocking conflict | all `CONFLICT` rows resolved explicitly | PASS only when final-head canonical guards pass; ordinary OWNER_AGENCY rent→4000 conflict is removed by the RC1 correction |
...
| G4 — Posting and reversal | included-scope owner-agency event/retry/reversal tests | PARTIAL: corrected OWNER/OFFICE invoice, legacy-payment fail-close, rent/fee tax lineage, target-bound credit reversal, governed VOID and period matrix are repository-proven; MASTER_LEASE is excluded; live/pilot proof pending |
| G5 — GL/subledger reconciliation | 1201/2000/1300/2200/2300 and included controls tie within 0.001 | PARTIAL: RC1 synthetic path proves all five controls at 0.000. Existing-company 2000 adoption requires approved S08 cutover opening balance; 1111/1120 tie to posted payment channel in the synthetic fixture, while real bank-statement match/live cycle proof remains pending |
...
- **GAP-006** corrected owner-agency invoice/collection/fee wiring: OWNER issuer stays operational until collection; OFFICE issuer posts 1201→2000 (+2100); RATE 1000.000→100.000→900.000 is derived from rent net of original tax; governed VOID/credit/reversal and 2000 event subledger are repository-proven. Hosted evidence remains pending.
...
- **GAP-010** versioned tax authority: fail-closed effective resolver now binds recurring invoices, collections and credits to immutable source snapshots; explicit configured `NON_TAXABLE` treatment replaces the legacy settings fallback; 3dp profile A→B credit proof passes. Statutory-code legal confirmation + hosted evidence remain pending.
- **GAP-013/014/015/016** deterministic reconciliation, GL-backed statements, S08 freeze, S09 engine: RC1 adds append-only `owner_funds_events` as the forward 2000 control basis and proves the synthetic 1201/1300/2000/2200/2300 table at 0.000. Accounting approval for S08/S09 and live cycle remain external.
...
- **GAP-011** included RC1 credit notes are source-economic/tax-lineage controlled. Per ADR 0017, late fees/other charges, unmapped general cash refunds and unmapped non-cash adjustments remain excluded/fail-closed pending DP-4; they are not silently mapped to an existing revenue account.
- **GAP-004/005** WP-03 engineering is complete for included RC1 scope. GAP-004: the canonical contract submit→approve→activate chain, draft-only renewal, company scope and immutable activation snapshot remain proven; owner-agreement and property creation atomically create the first explicit commercial version, and the workspace captures collection role/history/future amendments. GAP-005: backend-driven company-scoped onboarding, server-validated NON_WAIVABLE gates, audited waivers and append-only revoke/reset history remain proven. The approved Evidence & Compliance Journey adds empty-by-default effective registration profiles, registration submit + distinct verification + audit events, and structured move-in/out drafts/checklists/meters/keys/documents/signatures/distinct review. DAMAGE deposit requests now use the reviewed MOVE_OUT inspection boundary. This closes repository workflow absence without manufacturing Oman legal approval: GAP-019 remains external for the actual authority/deadline/fee/wording/retention profile; hosted acceptance remains GAP-020.
