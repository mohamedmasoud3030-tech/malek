...
- **GAP-004/005** WP-03 engineering. GAP-004 advanced to engineering-complete at repository level: the canonical submit→approve→activate chain is wired through service/UI, activation is the only path to 'active', `create_contract_atomic` is draft-only, and `update_contract_atomic` blocks direct activation plus commercial-term edits on active/APPROVED contracts; remaining are the `renew_contract_atomic` creation-status follow-up and legal template content (GAP-019). GAP-005 advanced: the onboarding checklist is now a backend-driven, company-scoped, audited workflow (templates + waivers + completion RPCs, NON_WAIVABLE identity/authority gates, admin-authorized waivers with reason) replacing localStorage; the exact canonical seven-step enumeration and any property-level safety evidence model remain a product decision (DP-5).
