...
- **GAP-004/005** WP-03 engineering is complete for included RC1 scope. GAP-004: the canonical contract submit→approve→activate chain, draft-only renewal, company scope and immutable activation snapshot remain proven; owner-agreement and property creation now atomically create the first explicit commercial version, the workspace captures collection role, displays immutable version history and creates future amendments only. GAP-005: backend-driven company-scoped onboarding, server-validated NON_WAIVABLE gates, audited waivers and append-only revoke/reset history remain proven. ADR 0017 adopts the implemented five-step order for RC1 and defers the later seven-step/property-safety expansion. Oman legal template/evidence approval remains independently GAP-019; hosted acceptance remains GAP-020.
