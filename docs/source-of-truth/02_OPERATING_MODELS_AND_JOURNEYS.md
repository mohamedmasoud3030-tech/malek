...
| B — tenant contract | Maker submits; distinct checker approves; authorized actor activates | valid company/tenant/property/unit; governing agreement version; signatures/evidence | `contracts`; approval/activation RPCs; immutable snapshots; effective-dated registration profile/record; move-in/out inspection drafts, signatures and distinct review | ACTIVE operational contract with immutable terms; registration status remains separately NOT_CONFIGURED/SUBMITTED/REGISTERED/REJECTED; reviewed handover evidence | reject/cancel/terminate through lifecycle; signed evidence is not overwritten; DAMAGE deposit claim requires a reviewed MOVE_OUT inspection in the new browser authority | Full repository chain is implemented; actual Oman registration profile/template approval and hosted evidence remain external |
