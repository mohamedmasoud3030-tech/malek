...
This is the same sequence `.github/workflows/ci.yml` runs (plus a `pnpm supabase:migration-evidence` preflight step before typecheck). CI sets placeholder `VITE_SUPABASE_URL`/`VITE_SUPABASE_ANON_KEY` values so the app builds without real credentials; do the same locally if you hit Supabase-config errors during build.
...
- **`pnpm lint`** — currently runs `tsc -p tsconfig.json --noEmit` inside `rentrix-app` (same as `typecheck`), not ESLint, even though `rentrix-app/eslint.config.js` exists. Treat "lint passes" as "typechecks," not as an ESLint guarantee, until this is reconciled — see `docs/CURRENT_STATE.md`.
...
- **`pnpm --filter ./rentrix-app test`** — runs the main Vitest suite: a fixed list of test files defined in `rentrix-app/package.json`'s `test` script (not a glob). If you add a new test file that should run in CI, add it to that list, or it may not execute even though it exists on disk.
...
- Any change to Supabase migrations, RPCs, or RLS policies: in addition to the standard suite, inspect the live Supabase project schema directly (see `docs/ARCHITECTURE.md` and `docs/CURRENT_STATE.md`) — passing local tests does not confirm a migration was applied correctly to a live database.
