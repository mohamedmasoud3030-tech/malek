...
The repository has a broad Playwright readiness matrix, but automation is not a substitute for the final operator sign-off on a real phone and desktop. The suite covers the three viewport classes, protected/public routing, Arabic RTL layout, high-traffic workspaces, accessibility checks, and fixture-backed authenticated journeys. Release Blocker adds an isolated Supabase write lifecycle and authenticated Production read-only proof. For UI changes, still verify the changed journey in a running `pnpm --filter ./rentrix-app dev` session and archive traces/screenshots for any failure. Never run a mutating financial journey against Production merely to obtain browser evidence.
