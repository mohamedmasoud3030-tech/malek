...
## الأولوية الحالية — إزالة مانع الإطلاق الحي لمكتب واحد

**Single-office launch: ليس GO بعد.** الفحص اليدوي الحي القراءة-فقط (2026-07-25، Asia/Muscat) كشف مانع قبول desktop/RTL؛ لا تبدأ أعمال multi-office أو النسخ الاحتياطي أو مرحلة جديدة قبل إغلاقه وإعادة الفحص الحي.

### P0 — تخطيط app shell على desktop RTL (مانع إطلاق)

- **إعادة الإنتاج:** سجّل الدخول بحساب ADMIN موجود على `https://rentrixapp.vercel.app` عند `1440×900`، ثم افتح `/invoices` أو `/reports` أو `/maintenance` أو `/settings`.
- **المشاهدة المثبتة:** الـsidebar الثابتة في اليمين تغطي مساحة `x=1169..1425` وبـ`z-index` أعلى، لكن `main` يمتد حتى `x=1425`. لهذا يقع عنوان الفواتير (`x=1309..1401`) وعناوين التقارير/الصيانة ومحتوى الطرف الأيمن تحت القائمة ولا تصل إليها المؤشرات/النقرات.
- **المطلوب:** تعديل تخطيط الـshell للـRTL بحيث تحجز `main` عرض الـsidebar من اليمين على desktop، ثم PR مستقل وsquash-merge وبعده إعادة نفس الفحص الحي القراءة-فقط على desktop/tablet/mobile. **لا تعديل على Production قبل موافقة Mohamed الصريحة لكل تغيير.**
- **لا تبدأ حلًا الآن:** هذه الملاحظة توثيقية من الفحص الحي وليست إذنًا بتغيير الكود أو النشر.

### ما تم إثباته دون كتابة على Production

- الصفحة العامة و`/login` يعيدان `200` مع `lang="ar" dir="rtl"`، وتسجيل دخول ADMIN موجود نجح.
- المساحات المطلوبة حمّلت: العقارات، الوحدات، العقود، الفواتير، الإيصالات، التقارير، الصيانة، الإعدادات. tablet (`768×1024`) وmobile (`390×844`) بلا horizontal overflow، وقائمة الهاتف تفتح؛ حالة الصيانة الصفرية عرضت CTA واضحًا.
- تصدير فاتورة PDF نجح محليًا، وزر طباعة الفاتورة فتح معاينة scoped A4؛ لم يُحفظ ناتج طباعة ولم يُنشأ/يُعدّل دفع أو إيصال أو VOID أو أي سجل أعمال.
- ظهر في console خط تحميل خارجي متكرر لـGoogle Fonts Cairo (`net::ERR_FAILED`) أثناء التنقل المحمي؛ لم يظهر خطأ تطبيق أو HTTP 4xx/5xx، وCSS نفسها أعادت `200` عبر curl وتحميل متصفح نظيف. يتابع كاعتمادية غير مانعة منفصلة.
- آخر migration في ledger هو `20260729091000` (تصحيح التوثيق القديم الذي توقف عند `20260729090000`).

بوابة القبول الآلية ما زالت على Supabase محلي مؤقت: ADMIN حقيقي + بيانات مترابطة + رحلة فاتورة → دفعة → إيصال → VOID وإثبات القيد العكسي. Production يبقى قراءة فقط أثناء الإثبات.
