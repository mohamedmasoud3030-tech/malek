...
- **Tenant** — a renter; has `phone`/`email`, `isArchived`. The canonical identity table for tenants across the application, contracts (`contracts.tenant_id`), receipt allocations (`receipt_allocations.tenant_id`), and financial balance summaries (`contract_balances.tenant_id`, `tenant_balances.tenant_id`) is `public.people` (where `type = 'tenant'`). The legacy `public.tenants` table is deprecated; `supabase/migrations/20260712020000_fix_tenant_balances_people_fk.sql` retargeted `tenant_balances.tenant_id` from legacy `tenants(id)` to `public.people(id)` (`ON DELETE RESTRICT`), preserving financial integrity and summary history without automatic `DELETE` cascading or silent data removal.
...
- **Invoice** — tied to a `LeaseContract`; has `amount`, `dueDate`, `status` (`unpaid` | `partially_paid` | `paid` | `overdue` | `cancelled`). Invoice generation creates double-entry accounting rows: debit tenant receivables, credit rental revenue, and credit VAT payable when tax applies. Duplicate generation is guarded and payment cadence follows the contract cycle.
- **PaymentReceipt** — a payment against an `Invoice`; has `amount`, `paymentDate`, `paymentMethod` (`cash` | `bank_transfer` | `check`), optional `referenceNumber`. Recording a payment uses `record_invoice_payment_atomic`; receipts are payment-backed, and voiding must preserve audit history while reversing the financial effect. Release evidence requirements live in `docs/RELEASE_READINESS.md`.
- **Expense** — tied to a `Property` and optionally a `Unit`; has `amount`, `expenseDate`, `description`, and `responsibility` (`owner` | `office` | `shared`). Creation and updates that affect accounting use guarded atomic RPC paths. Maintenance-driven expenses are created atomically alongside cost recording via `resolve_maintenance_with_expense`, which is restricted to ADMIN/MANAGER at the backend boundary.
...
- Every posted journal batch must balance total debits and credits.
- Posted journal entries are immutable.
- Corrections use explicit reversal entries; posted history is not deleted to “clean up” a mistake.
- Invoice, payment, expense, and void operations must update their accounting effect atomically.
- VOID/CANCELLED records remain available for history but are excluded from applicable totals.
- Financial foreign keys use RESTRICT or guarded lifecycle behavior where automatic cascading could destroy accounting history.
