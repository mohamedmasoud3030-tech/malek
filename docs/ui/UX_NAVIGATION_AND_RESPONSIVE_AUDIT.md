...
| `/expenses` | `ExpensesPage` | `rentrix-app/src/routeTree.ts`; `rentrix-app/src/routes/_protected.expenses.tsx`; `rentrix-app/src/features/financials/expenses/expenses-page.tsx` | Expenses financial page. | Expense list rows now stack into cards below `sm`; create-form and filter `Select`s are labeled. | Implemented intentionally |
| `/invoices` | `InvoicesPage` | `rentrix-app/src/routeTree.ts`; `rentrix-app/src/routes/_protected.invoices.tsx`; `rentrix-app/src/features/financials/invoices/invoices-page.tsx` | Invoice workspace sections. | Invoice/receipt lists already stack to labeled single-column rows below `md`/`lg`; quick-payment form now uses the shared `Input`/`Select` components (48px touch targets) instead of raw `min-h-10` fields. | Implemented intentionally |
...
| `/reports` | `ReportsPage` | `rentrix-app/src/routeTree.ts`; `rentrix-app/src/routes/_protected.reports.tsx`; `rentrix-app/src/features/reports/reports-page.tsx`; `rentrix-app/src/features/reports/components/*` | Report hub with report sections/tabs/filters. | `SectionTabs` (shared with `/settings`) already had a correct `role=tablist`/`overflow-x-auto`/hidden-scrollbar implementation; added a CSS mask-image edge fade so a 7-tab row signals it scrolls instead of looking cut off. | Implemented intentionally |
